# TODO – Eigenhandel-System

> **Stand:** 2026-08-24 · Das **Kundenprodukt ist eingestellt** (Entscheidung
> Max: starke Konkurrenzangebote, wirtschaftlich nicht tragfähig). Das System
> läuft künftig nur noch für Max' Eigenhandel. Gepflegt vom Developer Agent,
> gelesen von Max & Sepp.

## 🔴 Max – Abwicklung des Kundenprodukts

- [x] **`dailyoption.de` abgeschaltet (24.08.).** DNS-Einträge bei IONOS
      entfernt (autoritativ weder A noch AAAA, gegengeprüft über 1.1.1.1,
      8.8.8.8, 9.9.9.9), nginx-Site deaktiviert und nginx neu geladen.
      Sicherung der Site-Config: `/root/dailyoption.nginx.bak.2026-08-24`.
- [ ] **Stripe-Konto schließen.** Einstellungen → Unternehmen → Kontodetails
      → *Konto schließen* (**nicht** „Konto verlassen" — das entfernt nur dich
      als Nutzer, das Konto bliebe bestehen). Danach `STRIPE_SECRET_KEY` und
      `STRIPE_WEBHOOK_SECRET` aus `/etc/optionsbot/secrets.env` nehmen.
      Keine Vorarbeiten nötig: das einzige Abo (`sub_1TtYu0…`, 2 × 44,59 €)
      lief im **Testmodus** auf Max' eigenem Konto — kein echter Umsatz, keine
      Kündigung, kein Export, keine Rückerstattung.
- [ ] **`data/customers.db` löschen** (inkl. `-wal`/`-shm`). Reine Testdaten,
      aus dem Backup genommen, von keinem Dienst mehr benutzt.
- [ ] Inerte Env-Zeilen aufräumen: `CUSTOMER_BASE_URL`,
      `TELEGRAM_CUSTOMER_BOT_TOKEN` (werden nicht mehr gelesen).

## 🟡 Max – Betrieb (unverändert relevant)

- [ ] DNS-Zustellbarkeit der Morgenmail: SPF ergänzen, DKIM, DMARC `p=none`,
      Reverse-DNS
- [ ] Offsite-Backup (`BACKUP_REMOTE`), externer Uptime-Check, 2FA aktivieren

## 🟢 Developer Agent – Schritt 2

- [ ] Verbliebene Kundensprache in Kommentaren/Docstrings aufräumen
      (`bot/morning_routine.py`, `bot/formatting.py`, `bot/morning_pdf.py`)
- [ ] `steady-alpha-marketing`-Skill archivieren (gegenstandslos)
- [ ] `docs/morning_delivery.md` auf den Eigenhandel-Stand ziehen
- [ ] `MORNING_MAIL_RECIPIENTS` in `/etc/optionsbot/secrets.env` setzen, falls
      Max die Morgenmail an eine andere Adresse als `info@steadyalpha.de` will
- [ ] Frozen-Detection für Nicht-DD-Kennzahlen (`docs/verifier.md` §3)

---

# Archiv: GoLive-Roadmap Kundenprodukt (bis 2026-08-24)

> Ab hier steht der Stand vor der Einstellung. **Gegenstandslos**, bleibt nur
> als Nachvollziehbarkeit stehen.
