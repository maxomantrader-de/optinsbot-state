...
## 📍 Aktueller Fokus (Stand 2026-08-20)
...
- **Ausstiegsregelwerk vereinheitlicht (20.08., Entscheidung Max):**
  TP 50 % / Zeit-Exit 7 DTE / Stop 2× — Scanner-Planung und Überwachung meinen
  denselben Ausstieg, abgeleitet aus einer Quelle in `config/settings.py`.
...
- **Die Technik für den Launch steht.** Delivery-Strecke (Ziel 1) abgeschlossen,
  Verifier (Ziel 2) geklärt und abgegrenzt, Checkout rechtlich vollständig,
  Kundenstrecke erstmals testabgedeckt.
- Kunden-System live: Selbstregistrierung mit Double-Opt-in, Rollen (admin/customer),
  Pakete (morning/scanner), Landingpage, Profilseite, Abo-Seite, Portal-Archiv
- Morgenroutine läuft börsentäglich: 07:45 Abruf · 07:50 Gate · 07:55 PDF ·
  08:00 Versand (Mail + Telegram + Portal), Retry 08:20, Heartbeat 08:35
- Seither (12./13.08.): PDF-Design-Runde live, Abo-Selbstverwaltung inkl.
  zurücknehmbarer Kündigung, automatische Verlängerung durchgängig kommuniziert,
  Vola-Werte an die erwartete Session gebunden (fail-closed)
- Sync-Mirror: STATUS.md/TASKS.md/TODO.md → öffentliches Repo `optinsbot-state`
  (Schreibweise ohne zweites „o", s. o.)

## 🎯 Nächste Priorität
- **Liegt bei Max** (für nächste Woche verabredet): `STRIPE_PRICE_ID` anlegen,
  Stripe-Event `invoice.upcoming` im Webhook aktivieren, DNS-Zustellbarkeit
  (SPF **ergänzen**, DKIM, DMARC `p=none`, Reverse-DNS), Offsite-Backup,
  externer Uptime-Check, 2FA aktivieren, Beta-Nutzer finden
- **Agent, sobald PriceID vorliegt:** Stripe-E2E-Test im Testmodus
- **Agent, nicht launch-blockierend:** Frozen-Detection für Nicht-DD-Kennzahlen
  (`docs/verifier.md` §3), TL;DR + Mobil-Feinschliff auf `/morning`,
  Beispiel-Morgenmail als Vorschau
