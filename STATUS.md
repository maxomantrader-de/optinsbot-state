...
> **Sync:** Diese Datei (plus `TASKS.md` und `TODO.md`) wird per GitHub Action `mirror-state` automatisch ins öffentliche Read-only-Repo gespiegelt. **Adresse:** `github.com/maxomantrader-de/optinsbot-state` — ohne zweites „o", Tippfehler bei der Anlage; `optionsbot-state` existiert nicht (404). Der Mirror-README nennt oben Spiegelzeitpunkt und Quellstand. Keine sensiblen Daten hier eintragen.

---

## 📍 Aktueller Fokus (Stand 2026-09-15)
- **🚨 Kundenprodukt eingestellt (24.08., Entscheidung Max).** Begründung:
  starke Konkurrenzangebote, wirtschaftlich nicht tragfähig. Das System wird
  auf **reinen Eigenhandel** umgebaut. Schritt 1 ist umgesetzt: Kunden-
  verwaltung, Empfehlungsprogramm und Stripe sind vollständig entfernt,
  ebenso die Domain-Trennung und die gesamte Kundenstrecke. Es gibt nur noch
  die Rolle `admin`.
- **Scanner v2 ist aktiv (20.08.).** Neuentwicklung nach REGELWERK_STILLHALTER
  v2.0, kontogrößengetrieben, Signal-only. Scheduler-Lauf werktags 09:35 ET,
  Admin-Seite `/scanner` mit Kontogrößen-Eingabe. **Einstiegssignale wieder
  möglich.**
- **Portfolio aktuell leer:** die letzten beiden Positionen sind geschlossen
  (CL 70/68P am 17.08. +210,56 USD · GC 4080/4030P am 19.08. +519,96 USD).
- **Ausstiegsregelwerk (14.09., Take-Profit korrigiert 15.09.) — genau vier Regeln:**
  **Profit-Target 50 % · Break-Even · 7 DTE · 200-%-Regel.** Alle vier melden
  per Telegram, abgeleitet aus einer Quelle in `config/settings.py`.
  Quick-Win ist entfallen; der Delta-Watchdog meldet vorerst weiter als
  Frühwarnung (Entscheidung offen).
- **➡️ Zentrale To-do-Liste: [`TODO.md`](TODO.md)** — GoLive-Roadmap nach
  Verantwortlichkeit (Max / Developer Agent), dort gepflegt und mitgespiegelt.
- Morgenroutine läuft börsentäglich weiter: 07:45 Abruf · 07:50 Gate ·
  07:55 PDF · 08:00 Versand (Mail + Telegram), Retry 08:20, Heartbeat 08:35 —
  jetzt an feste Adressen statt an eine Empfängerliste aus der Datenbank
- Sync-Mirror: STATUS.md/TASKS.md/TODO.md → öffentliches Repo `optinsbot-state`
  (Schreibweise ohne zweites „o", s. o.)

## 🎯 Nächste Priorität
- **Abwicklung Kundenprodukt fast durch (24.08.):** `dailyoption.de` ist
  abgeschaltet (DNS bei IONOS entfernt, nginx-Site deaktiviert),
  `data/customers.db` gelöscht. Es gab **keine echten Kunden und keine echten
  Umsätze** — alle Konten und Zahlungen waren Max' eigene Tests im
  Stripe-Testmodus.
- **Offen:** Stripe-Konto schließen (Einstellungen → Unternehmen →
  Kontodetails → *Konto schließen*, nicht „Konto verlassen"), danach die
  `STRIPE_*`-Zeilen aus `/etc/optionsbot/secrets.env`. Dazu das
  Let's-Encrypt-Zertifikat `dailyoption.de` entfernen — ohne DNS scheitert
  die automatische Erneuerung ab ca. Mitte Oktober.
- **Liegt bei Max (Betrieb):** DNS-Zustellbarkeit (SPF, DKIM, DMARC,
  Reverse-DNS) für die Morgenmail, Offsite-Backup, externer Uptime-Check,
  2FA aktivieren.
- **Agent (Schritt 2):** verbliebene Kundensprache in Kommentaren und
  Docstrings aufräumen, `steady-alpha-marketing` archivieren, Frozen-Detection
  für Nicht-DD-Kennzahlen (`docs/verifier.md` §3).
- **Scanner:** v2 seit 20.08. aktiv. Nächster Schritt: erster Lauf innerhalb der
  Liquid Hours beobachten (bis dahin sind alle Läufe diagnostisch) und den
  Funnel über einige Tage sammeln, bevor an Parametern gedreht wird.
- **Beobachten:** Wirkung des 7-DTE-Zeit-Exits auf die ersten realen Trades —
  er hält Positionen deutlich tiefer in der Gamma-Zone als die früheren 21 DTE.
- **Zurückgestellt (bestätigt):** Trade-Management (Close/Roll)

## 🚧 Offene Blocker / Entscheidungen (für Max/Sepp)
- **Alle offenen Punkte jetzt in [`TODO.md`](TODO.md)** (einzige gepflegte Liste).
- [x] **Kundenprodukt** — eingestellt 2026-08-24 (Max): Konkurrenzlage, wirtschaftlich nicht tragfähig. System läuft als reines Eigenhandel-Werkzeug weiter.
...
### 2026-09-15 · UI/Developer Agent · Gateway-Schalter im Status-Strip neu gebaut
- Was geändert: Der Schalter las den Zustand aus `systemctl is-active` — der
  bleibt nach unbestätigtem 2FA-Push `active`, während Port 4001 tot ist; der
  Knopf stand dann auf Grün, obwohl kein Job verbinden konnte. Außerdem war
  ein Knopf zweideutig (grün = „läuft" **und** „klicken zum Stoppen").
  Jetzt: Zustand aus dem Port (`ib_gate.api_reachable`), drei Lagen
  grün/gelb/rot (verbunden · Dienst läuft ohne API · aus), Klappmenü mit
  Dienst-, Port- und Wartungsfenster-Stand und den Aktionen **Starten ·
  Neu starten · Stoppen · Überwachung fortsetzen**. Neustart = stop + start
  im Hintergrund-Thread (sudoers erlaubt kein `restart`), Doppelklicks werden
  abgewiesen. `/api/gateway` hält den Strip per Polling aktuell — Start und
  Neustart brauchen bis zu einer Minute, bisher musste man dafür von Hand neu
  laden. 2FA-Hinweis steht im Menü und in der Rückfrage. Der Strip erscheint
  jetzt auch auf `/logs` und `/rules`.
- Betroffene Dateien: `web/app.py`, `web/templates/base.html`,
  `bot/ib_gate.py`, `tests/test_gateway_switch.py`
- Status: fertig (600 Tests grün, Web-Dienst neu gestartet)
- Für Sepp relevant: ja — der angezeigte Gateway-Zustand war bisher nicht
  aussagekräftig; „Dienst aktiv" heißt nicht „API erreichbar".

### 2026-09-15 · Developer Agent · Hinweis „Scanner v1 zurückgebaut" von der Marktlage entfernt
- Was geändert: Der Untertitel der Seite „Marktlage" zeigte weiterhin „Scanner v1
  zurückgebaut, Neuentwicklung offen" — überholt, seit Scanner v2 am 20.08. aktiv
  ist. Die Zeile lautet jetzt nur noch „Marktfilter und Sektor-Lage".
- Betroffene Dateien: `web/templates/dashboard.html`
- Status: fertig
- Für Sepp relevant: ja — die Oberfläche behauptete an dieser Stelle noch einen
  Zustand von vor dem 20.08.

### 2026-09-15 · Developer Agent · Take-Profit zurück auf 50 %, Exit-Grund „manuell"

- **Was geändert (zwei Ansagen Max):**
  1. **Take-Profit 40 % → 50 %.** Einzige Änderung ist
     `EXIT_TAKE_PROFIT_PCT = 0.50` in `config/settings.py`; Regelwerk,
     Strategy Engine, Scanner-Planung und `ASSET_CONFIG` leiten ab, die
     Seiten `/rules` und `/positions` zeigen den Wert aus dem Context. Kein
     Wert steht doppelt.
  2. **Exit-Grund im Journal.** Dort stand der zuletzt gemeldete Alarm
     (`exit_alerted`), nicht der Grund der Schließung — an QQQ (04.08.,
     −657 USD) las sich das als „Quick-Win". Ein Grund, der zum Ergebnis
     nicht passt (Gewinn-Alarm bei Verlust und umgekehrt), gilt jetzt als
     von Hand geschlossen und erscheint als „✋ manuell"; ebenso ein leerer
     Grund, der vorher nur „—" war. Richtungsneutrale Gründe (Zeit-Exit,
     Delivery) bleiben unberührt.
- **Wirkung auf die Zahlen:** Positionen werden künftig später
  zurückgekauft (Rückkauf bei 50 % statt 60 % der Prämie) — mehr Ertrag je
  Trade, längere Haltedauer, höhere Gamma-Nähe zum 7-DTE-Exit hin. Die
  einzige offene Position (CL) hatte keinen aktiven Alarm, es geht dadurch
  keiner verloren.
- **`journal/trades.csv` bleibt unverändert** — dort steht weiterhin, welcher
  Alarm zuletzt aktiv war. Korrigiert ist die Aussage der Oberfläche, nicht
  die Aufzeichnung. 9 von 15 abgeschlossenen Trades zeigen jetzt „manuell".
- **Betroffene Dateien:** `config/settings.py`, `bot/ib_sync.py`,
  `bot/rules.py`, `bot/scanner.py`, `bot/sizing.py`, `web/app.py`,
  `web/templates/journal.html`, `tests/test_exit_regelwerk.py`,
  `tests/test_journal_pnl_pct.py`, `CLAUDE.md`
- **Status:** fertig (586 Tests grün) — Dienste müssen neu gestartet werden,
  damit Scheduler und Web-App den neuen Take-Profit laden
- **Für Sepp relevant:** ja — **Take-Profit ist wieder 50 %, nicht 40 %**;
  das Regelwerkspapier vom 14.09. ist an dieser Zahl überholt.

### 2026-09-14 · Developer Agent · Ausstiegsregeln abschließend: vier Regeln, Quick-Win raus

- **Was geändert (Ansage Max):** „Ich möchte weiterhin per Telegram informiert
  werden, wenn die Exit Regeln erreicht werden. Exit Regeln sind aber nur
  Profit-Target, Break-Even-Exit, 7 Tage DTE und 200 % Regel."
  - Damit ist der Satz „Kein Stop-Loss / kein Break-Even-Exit" aus dem
    Regelwerkspapier vom selben Tag **korrigiert**: Beide gelten.
  - **Quick-Win entfernt** (`check_exit`, `QUICK_WIN_PCT/_DAYS`): Er schloss
    bei 25 % Gewinn in den ersten 7 Tagen — also vor dem Take-Profit bei 40 %.
    Alte Trades im Journal tragen den Grund weiter, die Anzeige kennt ihn.
  - **Delta-Watchdog bleibt vorerst** und steht auf `/rules` getrennt als
    „Zusätzliche Frühwarnung": Er meldet bei |Δ| > 0,50 (Short-Strike am Geld)
    und damit früher als der Break-Even-Bruch. Entscheidung über Verbleib offen.
  - `/rules`: eigener Abschnitt **Ausstieg** mit den vier Regeln und dem
    Hinweis, dass jede per Telegram meldet. `/positions` zeigt jetzt alle vier.
  - Telegram-Text des Delta-Watchdogs umgestellt: Kopf „⚠️ Frühwarnung", Fuß
    „Kein Ausstiegsgrund – nur im Blick behalten" statt „Bitte in IBKR manuell
    schließen". Vorher las sich die Warnung wie eine fünfte Ausstiegsregel.
  - Liste „Bewusst gestrichen" auf `/rules` wieder entfernt (Ansage Max): Die
    Seite zeigt, was gilt, nicht was einmal galt.
- **Betroffene Dateien:** `bot/rules.py`, `config/settings.py`, `web/app.py`,
  `web/templates/rules.html`, `web/templates/positions.html`,
  `tests/test_exit_regelwerk.py`, `tests/test_rules_page.py`
- **Status:** fertig — 578 Tests grün, Dienste neu gestartet
- **Für Sepp relevant:** ja — verbindlich sind ab jetzt genau vier
  Ausstiegsregeln: Profit-Target 40 %, Break-Even, 7 DTE, 200 %.

### 2026-09-14 · Developer Agent · Regelwerk-Seite auf v2.0, Web-UI entrümpelt

- **Was geändert (Ansage Max: Webseite an die Parameter anpassen, Infos aufs
  Nötigste reduzieren):**
  - `/rules` komplett neu: zeigt jetzt **das geltende Regelwerk Stillhalter
    v2.0**, vollständig aus `config/settings.py` abgeleitet — Budget und
    Risikostufen (4 % Standard · 6 % nur bewusst gewählt), Kernparameter
    (Short-Δ 0,30 · Long-Δ 0,10 / MES 0,15 · 45 DTE · IVR ≥ 40), Richtung über
    MA200, Regime-Staffelung, Portfolio-Grenzen, Universum.
  - Die alte Quelle der Seite — 32 von Hand gepflegte Regeltexte der Rules
    2026_01 (`RULE_DESCRIPTIONS`) — ist **gelöscht**. Sie zeigte noch Delta
    0,18, DTE 50–60 und Take-Profit 60 %, während der Scanner nach v2.0 lief.
  - **Take-Profit 50 % → 40 %** in der einen Quelle (`EXIT_TAKE_PROFIT_PCT`);
    Scanner, Überwachung, Strategy Engine und Positionsseite ziehen mit.
  - Doppelte Darstellungen entfernt: Regelwerk-, Ausstiegs- und
    Universums-Tabellen auf `/scanner`, Regime-Staffelung dort ebenfalls —
    `/scanner` zeigt jetzt Kontogröße, Kandidaten, Funnel und verlinkt die
    Parameter.
  - Sektor-Texte der Marktlage rein zustandsbeschreibend: die zweite
    Textfassung nannte Handelssperren aus den Rules 2026_01 („kein neuer
    /CL-Trade (Regel 013)"), die der Scanner v2 nicht kennt.
  - Veralteter Kasten „Scanner v1 zurückgebaut / in Neuentwicklung" von der
    Marktlage entfernt; Pflichtangaben für Dritte aus `/morning` und
    `/methodik` entfernt (Mail und PDF behalten sie).
- **Betroffene Dateien:** `web/app.py`, `web/templates/rules.html`,
  `scanner.html`, `dashboard.html`, `morning.html`, `methodik.html`,
  `login.html`, `config/settings.py`, `bot/notifier.py`,
  `tests/test_rules_page.py` (neu), `tests/test_exit_regelwerk.py`,
  `tests/test_scanner_page.py`
- **Status:** fertig — 574 Tests grün
- **Offen / Entscheidung Max:** Das Regelwerk streicht **Stop-Loss, Rollen,
  Break-Even-Exit, Quick-Win und Delta-Watchdog**. Im Code melden diese
  Backstops weiter per Telegram; ihr Ausbau greift in die Überwachung offener
  Positionen ein und wurde bewusst **nicht** mitgemacht. `/rules` weist sie
  unter „Überwachung" als noch aktiv aus, damit Seite und Verhalten
  übereinstimmen. Ein Wort von Max genügt, dann fallen sie.
- **Für Sepp relevant:** ja — Take-Profit liegt jetzt bei 40 %, und die
  Regelwerk-Seite ist keine eigene Textquelle mehr, sondern spiegelt
  `settings.py`.

### 2026-08-26 · Developer Agent · Rückbenennung Steady Ground → Steady Alpha

- **Was geändert (Ansage Max):** „Steady Ground" war der Name für die
  Kundenstrecke (Umbenennung 14.08.). Mit deren Einstellung ist er
  gegenstandslos — zurück auf **Steady Alpha**, das auch zur Domain
  `steadyalpha.de` passt. Betroffen: `PRODUCT_NAME`, alle Web-Seiten, PDF,
  Mail, Telegram-Bild und -Text, Skripte, Skills, Doku.
- **Ursache mitbehoben:** Der Name stand in **acht Templates fest verdrahtet**
  (Seitentitel, Login, Regelwerk-Kopf) — entgegen der eigenen Regel, dass er
  aus dem Flask-Context kommt. Jetzt überall `{{ product_name }}`; die nächste
  Umbenennung ist damit eine einzige Zeile in `config/settings.py`.
- **Riskante Stelle, bewusst behandelt:** Das Tagesartefakt heißt jetzt
  `YYYYMMDD_steadyalpha_mr.pdf`. Der bisherige Slug `steadyground_mr` steht in
  `_LEGACY_SLUGS` — **auf der Platte lagen drei echte PDFs damit** (24.–26.08.),
  die sonst nicht mehr gefunden und still neu gerendert worden wären. Neuer
  Test `test_stored_artifact_findet_die_steadyground_ausgaben` hält das fest.
  Ebenso unangetastet: `_LEGACY_STEMS` und die beiden Backup-Globs in
  `scripts/backup.py` — Alt-Namen dürfen dort nur wachsen, nie verschwinden.
- **Skills umbenannt:** `steady-ground-{marketing,ui,security}` →
  `steady-alpha-*` (Verzeichnisse per `git mv`, Inhalte und alle Verweise in
  `CLAUDE.md` nachgezogen).
- **Historie bleibt stehen:** Die Changelog-Einträge unterhalb nennen weiter
  „Steady Ground", wo sie den damaligen Stand beschreiben — aus „Umbenennung
  Steady Alpha → Steady Ground" (14.08.) würde sonst eine Falschaussage.
- **Betroffene Dateien:** `config/settings.py`, `bot/morning_pdf.py`,
  8 Templates, `scripts/{backup,admin_totp_setup,install_fonts}.py`,
  `docs/morning_payload.{schema,example}.json`, `docs/morning_delivery.md`,
  `deploy/50-optionsbot-ibgateway.rules`, `web/static/{favicon.svg,css/morning_pdf.css}`,
  `bot/formatting.py`, 3 Testdateien, `.claude/skills/*`, `.claude/agents/*`,
  `CLAUDE.md`, `TODO.md`, `AUDIT_MORGENROUTINE.md`
- **Status:** fertig — 567 Tests grün, beide Dienste neu gestartet,
  Login-Seite und Telegram-Bild führen den neuen Namen, alle bestehenden PDFs
  weiterhin auffindbar (geprüft für 06.08. bis 26.08.).
- **Für Sepp relevant:** ja — das Produkt heißt ab sofort wieder Steady Alpha.

### 2026-08-26 · Developer Agent · Telegram-Morgenroutine: Kunden-Ballast entfernt

- **Was geändert (Ansage Max):** Die Morgenroutine geht nur noch an Max selbst.
  Aus der Telegram-Nachricht sind damit die an Dritte gerichteten Teile raus:
  - **Pflichtangaben-Block** komplett (Haftungsausschluss „keine
    Anlageberatung", Erstellerangabe „Neuner Productions, keine BaFin-Aufsicht",
    Interessenkonflikt, Verweis auf `/methodik`). Sich selbst mitzuteilen, dass
    man keine Anlageberatung erhält und dass der Ersteller eigene Positionen
    hält, ist gegenstandslos.
  - **Bild-Fußzeile** auf die reine Herkunftsangabe reduziert
    („Steady Ground · Ausgabe JJJJ-MM-TT"); Haftungssatz und Werbeadresse
    `steadyalpha.de` sind raus.
  - Wort „Pflichtangaben" aus dem PDF-Hinweissatz entfernt.
  - **Behalten:** Ampel, Kennzahlen, Datenstand und der Link „Zur ausführlichen
    Analyse" — die nützen Max.
- **Wirkung:** Kennzahlen-Nachricht von rund 1.600 auf **924 Zeichen**.
- **Test invertiert:** `test_telegram_ohne_pflichtangaben` hält jetzt fest,
  dass der Block **draußen bleibt** — er kann nicht unbemerkt zurückkriechen.
- **Bewusst nicht angefasst:** **Mail und PDF tragen die Pflichtangaben
  weiter.** Der Auftrag galt der Telegram-Nachricht; für die beiden anderen
  Wege ist es ein eigener Auftrag (`bot/morning_mail.py`,
  `bot/morning_payload.py`, `web/templates/morning_pdf_macros.html`).
- **Betroffene Dateien:** `bot/morning_telegram.py`, `tests/test_morning_view.py`
- **Status:** fertig — 566 Tests grün, Scheduler neu gestartet.
- **Für Sepp relevant:** ja — die tägliche Telegram-Ausgabe sieht ab dem
  27.08. anders aus.

### 2026-08-26 · Developer Agent · Gateway-Backoff im Ernstfall bestätigt + Doku korrigiert

- **Erster echter Test der Umstellung, Nacht 25./26.08.:** Max stoppte das
  Gateway am 25.08. gegen 15:30 über die Web-UI (Anmeldung in der IBKR-App).
  Nach Ablauf des Wartungsfensters startete der Watchdog um 20:30 neu, der
  2FA-Push blieb abends unbestätigt. Ergebnis: **genau 1 Push** statt der 24
  vom 24.08. IBC protokollierte „Re-login … not required" und legte nicht
  nach; der Backoff ging auf 240 Min, der neue 2FA-Hinweis ging um 20:34
  einmalig per Telegram raus. **Die Maßnahme wirkt wie beabsichtigt.**
- **Doku korrigiert (Beobachtung widerspricht der Annahme):** `ExitAfter…=yes`
  löst in diesem Pfad **keinen** Prozessausstieg aus. IBC bleibt stehen, der
  Dienst gilt weiter als `active` — nur ohne offenen Port 4001. Ungefährlich
  (keine Push-Flut), der Watchdog räumt es per `stop`+`start` weg. Folge für
  die Diagnose: **`systemctl is-active ibgateway` sagt nichts über die API**,
  es zählt allein `ss -tlnp | grep :4001`. In CLAUDE.md festgehalten.
- **Ebenfalls festgehalten:** Außerhalb der US-Handelszeit greift niemand ans
  Gateway (Watchdog hängt an Strategy Engine/IB-Sync 09–16 ET + Prewarm
  09:15 ET). Ein abendlicher Ausfall bleibt bis zum Prewarm bestehen — gewollt,
  sonst wäre ein Neustart um 3 Uhr nachts ein Push um 3 Uhr nachts.
- **Betroffene Dateien:** `CLAUDE.md`, `STATUS.md`
- **Status:** fertig — Gateway am 26.08. um 09:53 neu gestartet, Anmeldung nach
  17 s bestätigt, Port offen, Backoff auf 0 zurückgesetzt, Watchdog meldet OK.
- **Für Sepp relevant:** ja — belegt, dass die Push-Flut vom 24.08. behoben ist.

### 2026-08-25 · Developer Agent · Liegengebliebenen Telegram-Fix nach main geholt

- **Was geändert:** Beim Aufräumen fiel ein Worktree
  (`.claude/worktrees/mystifying-allen-4399fe`) mit einem Commit vom 19.08.
  auf, der **nie in `main` gelandet** war: Lange Zustands-Labels („Erhöhte
  Aufmerksamkeit", „Datenlage unvollständig") liefen im börsentäglichen
  Telegram-Bild aus dem Lampenkasten über den Bildrand und in die Spalte
  „Positionierung". Neuer Helfer `_fit_lines()` bricht auf höchstens zwei
  Zeilen um und senkt den Schriftgrad erst, wenn ein einzelnes Wort für sich
  zu breit bleibt; die Sperrung skaliert mit dem Grad mit. Kurze Labels
  („UNAUFFÄLLIG", „ANGESPANNT") bleiben unverändert einzeilig bei Grad 13.
- **Geprüft, nicht bloß übernommen:** Der Fehler steckte in `main` noch drin
  (alte einzeilige Fassung). Gegenprobe gemacht — der mitgelieferte
  Regressionstest misst die Tinte pixelweise und **schlägt auf dem alten
  Stand fehl**, genau bei den langen Labels, während die kurzen durchgehen.
- **Betroffene Dateien:** `bot/morning_telegram.py`, `tests/test_morning_view.py`
- **Status:** fertig — 566 Tests grün (6 neue), Scheduler neu gestartet.
  Worktree und Branch `claude/mystifying-allen-4399fe` danach entfernt.
- **Für Sepp relevant:** ja — das Tagesbild sah seit dem 19.08. an jedem Tag
  mit langem Zustands-Label fehlerhaft aus, obwohl der Fix längst geschrieben war.

### 2026-08-25 · Developer Agent · Anmelde-Schleife am IB Gateway entschärft

- **Befund:** Am Abend des 24.08. meldete sich das Gateway zwischen 19:05 und
  23:46 (Berlin) **41×** bei IBKR an und löste dabei **24 Zwei-Faktor-Pushes**
  auf Max' Handy aus, bis IBKR mit „Too many failed login attempts" drosselte.
  Ursache war eine Regel ohne Obergrenze: `ReloginAfterSecondFactor…=yes` ließ
  IBC nach jedem 2FA-Timeout im selben Prozess sofort neu anmelden. Ausgelöst
  hatte es ein gewollter Stopp über die Web-UI (Login in der IBKR-App); das
  90-Minuten-Wartungsfenster lief ab, während die App-Sitzung noch stand.
  Ähnliche, kleinere Häufungen am 21.08. (16 Pushes) und 13.08. (8).
- **Was geändert:**
  - IBC legt nach 2FA-Timeout nicht mehr nach (`ReloginAfter…=no`,
    `ExitAfter…=yes`, Exit-Intervall 60 s, explizit gesetzt statt auf
    Vorgaben vertraut).
  - **Backoff im Verbindungs-Gate:** vergebliche Neustarts verdoppeln das
    Intervall (60 → 120 → 240 → max. 480 Min); eine bestätigte Verbindung
    setzt zurück. Neu: `GATEWAY_RESTART_BACKOFF_MAX_MIN`, `…_ALERT_AFTER`.
  - **Neue Telegram-Meldung**, genau einmal je Ausfall: benennt die
    unbestätigte 2FA als wahrscheinliche Ursache und sagt, was zu tun ist.
    Der bisherige Alarm sagte nur, *dass* etwas kaputt ist.
  - `Restart=on-failure` in der systemd-Unit auf `Restart=no` — die Regel war
    ohnehin wirkungslos (`tee` ohne `pipefail` maskiert den Exit-Code auf 0),
    wäre nach der Umstellung aber zur Falle geworden: alle 30 s ein Neustart
    und damit ein Push. Neustarts macht ausschließlich der Watchdog.
- **Aufgeräumt:** `/etc/ibgateway/credentials` enthielt eine **zweite, von
  niemandem gelesene Kopie** von Benutzername und Passwort (IBC nimmt beides
  aus `config.ini`; das Startskript übergibt weder `--user/--pw` noch setzt es
  `TWSUSERID`). Entfernt, nur `IB_TRADING_MODE` bleibt. Außerdem erstmals eine
  logrotate-Regel fürs Gateway-Log (lief seit dem 30.03. ohne Rotation auf
  2,4 MB; `copytruncate` wegen des offenen `tee`-Deskriptors).
  Gegengeprüft: Im Log steht das Passwort **nur maskiert** (`***`).
- **Betroffene Dateien:** `bot/ib_gate.py`, `bot/gateway_watchdog.py`,
  `config/settings.py`, `tests/test_ib_gate.py` · außerhalb Git:
  `/opt/ibc/config.ini`, `/etc/systemd/system/ibgateway.service`,
  `/etc/ibgateway/credentials`, `/etc/logrotate.d/ibgateway`
- **Status:** fertig — 560 Tests grün, Scheduler und Web neu gestartet,
  Gateway durchgehend verbunden (kein Neustart nötig, kein Push ausgelöst).
- **Offen bei Max:** Zweiter IBKR-Benutzername ist beantragt, Freischaltung
  morgen. Neue Zuordnung: **App = neuer Benutzer, Server behält den
  bisherigen** — damit bleiben die Marktdaten-Abos beim Gateway. Der neue
  Benutzer braucht **Handelsberechtigung** (Max ordert von Hand darüber).
  Danach entfallen Gateway-Stopp und Wartungsfenster im Alltag.
- **Für Sepp relevant:** ja — erklärt die Gateway-Ausfälle vom 24.08. und
  warum Scans/Monitore an dem Abend ausfielen.

### 2026-08-24 · Developer Agent · Positionsseite: Kurs, vereinnahmtes Theta, Delta

- **Was geändert:** Auftrag Max — `/positions` als tabellarische
  Arbeitsansicht mit drei neuen Größen je Position: aktueller Basiswertkurs,
  bisher vereinnahmtes Theta, aktuelles Short-Delta (mit Rückfall auf den
  letzten Abruf). Dazu Puffer bis Break-Even, Theta-Rate, Rückkaufwert, G/V,
  DTE. Karten-Layout → Tabelle; unter 576 px wird jede Zeile zu einem Block
  mit „Label: Wert" (neue Variante `.data-table.stack-sm` in `base.html`).
- **Datenschicht:** `Position` um Kurs, Delta, Rückkaufwert und deren
  Zeitstempel erweitert; neue `TradeJournal.update_market_snapshot()` schreibt
  die Momentaufnahme aus dem Monitor-Lauf fort. `None` überschreibt nie einen
  vorhandenen Wert — der letzte bekannte Stand bleibt mit seinem Alter stehen.
- **Vereinnahmtes Theta** gibt es am Markt nicht abzulesen (Theta ist eine
  Rate). Es wird zwischen zwei Messungen integriert (Trapez × Kalendertage),
  Zähler startet mit der ersten Messung, einzelne Lücken auf 5 Tage gedeckelt.
  Bewusst NICHT `Credit − Rückkaufwert` — das wäre das gesamte G/V inklusive
  Kursbewegung, nicht der Zeitwertanteil.
- **Neuer Knopf „Marktdaten aktualisieren"** (`/positions/refresh`): ruft die
  Werte im Hintergrund-Thread ab, eigene IB-Client-ID 27, nur lesend, kein
  Telegram, kein zweiter Lauf parallel.
- **Zwei Fehler beim Testen gefunden und behoben:** (1) Fehlte einer Position
  ein Feld, brach die ganze Seite mit einem Jinja-UndefinedError ab — die
  Kopfsummen werden jetzt in Python gebildet, nicht per `sum(attribute=...)`.
  (2) Zahlenspalten hatten in der Stapel-Ansicht `nowrap` und rissen die Karte
  auf dem Telefon seitlich auf.
- **Betroffene Dateien:** `bot/rules.py`, `journal/trades.py`, `bot/manager.py`,
  `web/app.py`, `web/templates/positions.html`, `web/templates/base.html`,
  `config/settings.py`, `tests/test_positions_view.py` (neu, 18 Tests)
- **Status:** fertig — 554 Tests grün, Dienste neu gestartet, Seite live.
  **Noch offen: der Live-Abruf gegen IB ist ungetestet.** Das Gateway lag
  während der Arbeit im Wartungsfenster (bewusst gestoppt, Watchdog pausiert).
  Erster echter Test: Knopf drücken, sobald das Gateway wieder läuft — oder
  automatisch beim nächsten Monitor-Lauf.
- **Bekannte Einschränkung (älter als diese Änderung):** `positions.json` wird
  ohne Sperre gelesen/geschrieben; Monitor, IB-Sync und der neue Knopf können
  sich theoretisch überschreiben. Bisher nie beobachtet, aber ein Kandidat für
  eine Dateisperre.
- **Für Sepp relevant:** ja — neue Kennzahl „vereinnahmtes Theta" je Position,
  Definition steht in CLAUDE.md.

### 2026-08-24 · Developer Agent · Kundenprodukt entfernt (Kundenverwaltung, Empfehlungen, Stripe)

- **Was geändert:** Auftrag Max — das Kundenprodukt wird eingestellt, das
  System läuft künftig nur noch für den Eigenhandel. Schritt 1 entfernt drei
  Bereiche vollständig **plus** alles, was dadurch tot oder irreführend wurde:
  - **Kundenverwaltung:** Registrierung, Double-Opt-in, Kunden-Login, Profil,
    Zustellwahl, `/admin/customers*`, Datenmodell `web/models.py`
    (Customer/Subscription/Invoice).
  - **Empfehlungen:** `ReferralPartner`/`ReferralCommission`,
    `/admin/referrals*`, Codes, Rabatte, `/partnerbedingungen`.
  - **Stripe:** Checkout, `/stripe/webhook`, alle `_wh_*`-Handler, Abo-Sync,
    `PRICING`, Webhook-CSRF-Ausnahme, eigenes Rate-Limit, Paket `stripe`.
  - **Folgeabbau:** Kundenstrecke (Landing, `/abo`, `/kuendigen`,
    `/no-access`, `/beispiel`, Rechtstexte), Domain-Trennung (`host_routing`,
    Endpunkt-Registries, `CUSTOMER_BASE_URL`, Produktname „Daily Option"),
    Rollen `customer`/`package_required`, SQLAlchemy-Abhängigkeit.
- **Entscheidungen (autonom, dokumentiert):**
  - **Morgenroutine bleibt** — sie ist Max' Werkzeug, nicht nur Produkt.
    Empfänger kommen jetzt aus `MORNING_MAIL_RECIPIENTS` (Default
    `ADMIN_CONTACT_EMAIL`) bzw. `TELEGRAM_CHAT_ID` über den Admin-Bot.
  - **Domain-Trennung mit entfernt:** Ohne Kundenrouten hätte sie nur noch die
    Marke „Daily Option" in Mail, PDF und Oberfläche gehalten.
  - **PDF-Artefakt umbenannt** `_dailyoption_mr` → `_steadyground_mr`; alte
    Dateien bleiben über `_LEGACY_SLUGS` auffindbar.
  - **`data/customers.db` NICHT gelöscht**, nur aus dem Backup genommen — über
    personenbezogene Daten entscheidet Max.
- **Betroffene Dateien:** `web/app.py` (3.716 → 1.541 Zeilen), `web/models.py`
  (gelöscht), 16 Templates gelöscht, `web/templates/base.html`,
  `web/templates/morning.html`, `config/settings.py`, `bot/morning_mail.py`,
  `bot/morning_telegram.py`, `bot/morning_payload.py`, `bot/morning_pdf.py`,
  `bot/morning_delivery.py`, `scheduler.py`, `scripts/backup.py`,
  `requirements.txt`, Tests (3 gelöscht, 4 angepasst), `CLAUDE.md`
- **Status:** fertig — 536 Tests grün, alle verbliebenen Routen liegen hinter
  `@admin_required` (Ausnahmen: `/login`, `/healthz`, `/sepp/*`).
- **Noch offen (Max):** nginx-Site + DNS `dailyoption.de` abschalten — **ohne
  Host-Routing bedient diese Domain jetzt die Admin-Oberfläche**; Stripe-Konto
  deaktivieren; Löschung der Kundendatenbank entscheiden.
- **Für Sepp relevant:** ja — die gesamte Kunden-/Abo-/Empfehlungs-Domäne ist
  weg, Aufgaben und Roadmap-Punkte dazu sind gegenstandslos.
- **Nachtrag 24.08.:** Max bestätigt, dass alle drei Konten in der Datenbank,
  das Stripe-Abo und beide Zahlungen (2 × 44,59 €) seine **eigenen Testkonten
  im Stripe-Testmodus** waren. Kein echter Umsatz, keine Betroffenenrechte,
  keine Aufbewahrungspflicht — `data/customers.db` kann ersatzlos weg, das
  Stripe-Konto ohne Vorarbeiten geschlossen werden.

### 2026-08-20 · Developer Agent · Morgenroutine-PDF: Kurve vollständig, neuer Dateiname

- **Was geändert:**
  1. **VIX-Terminkurve war oben abgeschnitten.** Die Zahl über dem höchsten
     Punkt saß auf einer Grundlinie bei 0,6 Einheiten — ihre Oberlänge ragte
     aus der viewBox, im PDF fehlte die obere Hälfte der hinteren Terminmonate
     (zuletzt „21,5" und „21,9"). Ursache war ein fester Einzug von 3 Einheiten,
     unabhängig von der Schriftgröße. Die Randabstände werden jetzt aus den
     Schriftmaßen **berechnet** (`SPARK_VALUE_SIZE`, Ober-/Unterlänge,
     Zeichenbreite); das Makro liest die Schriftgröße aus demselben Ergebnis —
     eine Quelle für Maß und Darstellung. Zeichenfläche 26 → 30 Einheiten hoch,
     damit die Kurve trotz des größeren Einzugs ihre Amplitude behält.
  2. **Dateiname nach Vorgabe Max:** `YYYYMMDD_dailyoption_mr.pdf` statt
     `steady-ground-morgenroutine-YYYY-MM-DD.pdf`. Einzige Quelle ist
     `morning_pdf.artifact_filename()`; Archiv, Mail-Anhang, Telegram-Dokument
     und Portal-Download lesen von dort. Der Bestand (10 Tage) wurde umbenannt,
     die alten Namen bleiben als Fallback für Restores aus Backups.
- **Betroffene Dateien:** `bot/morning_pdf.py`, `web/templates/morning_pdf_macros.html`,
  `bot/morning_mail.py`, `bot/morning_telegram.py`, `web/app.py`,
  `tests/test_morning_pdf.py`, `tests/test_morning_delivery.py`
- **Geprüft:** PDF aus dem Tagesarchiv 20.08. gerendert — alle sechs
  Terminmonate vollständig, Layout weiter exakt 2 Seiten. Neuer Regressionstest
  prüft die Beschriftung als Fläche (Ober-/Unterlänge, Breite) für steigende,
  fallende und flache Kurven. Gesamte Suite: 645 grün, 1 übersprungen.
- **Status:** fertig
- **Für Sepp relevant:** ja — der Anhang der Morgenroutine heißt ab sofort
  anders, und die Terminkurve im Kundendokument ist erstmals vollständig
  lesbar.
- **Nachtrag 21.08.:** Die Ausgabe vom 20.08. lag noch mit der abgeschnittenen
  Kurve im Archiv (gerendert 05:55, versendet 07:45). Auf Ansage Max neu
  gerendert und ersetzt — der Portal-Download zeigt jetzt auch für diesen Tag
  die vollständige Kurve. Datum, Datenstand und Erstellungszeit stammen
  unverändert aus dem Tagesarchiv, es ist dieselbe Ausgabe. Ältere Ausgaben
  bleiben, wie sie ausgeliefert wurden. Der planmäßige Lauf vom 21.08. hat
  Kurve und Dateinamen von sich aus korrekt erzeugt.

### 2026-08-20 · Developer Agent · Telegram-Kanal zurückgebaut (Entscheidung Max)

- **Was geändert:** Der wenige Stunden zuvor gebaute Telegram-Kanal ist wieder
  raus. Max: „passt so wie es ist. Jeder User bekommt es in seinen Chat wie
  gehabt." Die Kundenzustellung war davon ohnehin nie berührt — der Kanal war
  ein zusätzlicher Weg obendrauf, kein Ersatz.
- **Entfernt:** `TELEGRAM_CHANNEL_ID`/`TELEGRAM_CHANNEL_ACTIVE`,
  `notifier.send_channel()` samt Signal-Routing, `post_morning_to_channel()`,
  der Aufruf in `deliver_day` (inkl. `kanal`-Schlüssel im Ergebnis),
  `scripts/telegram_kanal_id.py`, `tests/test_telegram_kanal.py` und die
  zugehörigen Abschnitte in CLAUDE.md und TODO.md.
- **Bewusst vollständig zurückgebaut** statt inaktiv stehen gelassen: Ein
  ungenutzter Zweig plus eine rote To-do-Zeile, die zum Einrichten auffordert,
  wäre irreführender als kein Code. Die Git-Historie hält ihn fest, falls er
  doch nochmal gebraucht wird (Commit `b728c13`).
- **Bleibt bestehen:** Scan-Zeitpunkt **10:00 ET** (30 Min nach Handelsstart)
  samt `SCAN_SCHEDULE_ET_*` und der abgeleiteten deutschen Uhrzeit auf der
  Scanner-Seite. Die beiden Tests dazu sind nach
  `tests/test_scanner_page.py` umgezogen.
- **Zustellung jetzt wie zuvor:** Morgenroutine → Mail + persönlicher
  Telegram-Chat je Kunde (@SteadyAlphaServiceBot) · Scanner-Signale und
  Betriebsmeldungen → Admin-Bot (@maxoptions1bot).
- **Betroffene Dateien:** `config/settings.py`, `bot/notifier.py`,
  `bot/morning_telegram.py`, `bot/morning_delivery.py`, `CLAUDE.md`, `TODO.md`,
  `tests/test_scanner_page.py`, `tests/test_morning_delivery.py`.
- **Verifiziert:** Suite **643 passed / 1 skipped / 0 failed**.
- **Status:** fertig
- **Für Sepp relevant:** ja — die Kanal-Ankündigung aus dem Eintrag darunter
  gilt nicht mehr.


### 2026-08-20 · Developer Agent · Scan 30 Min nach Handelsstart + Telegram-Kanal

- **Scan-Zeitpunkt:** 09:35 → **10:00 ET** (30 Minuten nach der US-Eröffnung,
  Auftrag Max). In den ersten Minuten nach der Eröffnung sind die Spreads noch
  weit und die Greeks unruhig. Zeitpunkt jetzt in `SCAN_SCHEDULE_ET_*`
  konfiguriert; die Oberfläche rechnet die deutsche Entsprechung (derzeit
  **16:00 Uhr**) aus, statt sie als Text zu pflegen — USA und Europa stellen die
  Uhr an verschiedenen Terminen um.
- **Telegram-Kanal „steadyalpha":** Scanner-Kandidaten und Morgenroutine gehen
  jetzt dorthin, über **@SteadyAlphaServiceBot** (derselbe Bot wie die
  Kundenzustellung — ein Bot kann in beliebig vielen Kanälen posten).
  Betriebsmeldungen an Max bleiben auf dem Admin-Bot.
- **Fail-closed:** Ohne `TELEGRAM_CHANNEL_ID` wird nichts ersatzweise an den
  Admin-Chat gesendet. Einzige Ausnahme: ein gefundener Scanner-Kandidat fällt
  auf den Admin-Bot zurück, damit er nicht an einer fehlenden Konfigurationszeile
  verloren geht.
- **Morgenroutine im Kanal:** Vorschaubild, Fazit und Kennzahlen in der neutralen
  Fassung. **Das PDF geht bewusst nicht mit** — es ist das Kundenprodukt.
  Veröffentlichung läuft nach der Kundenzustellung und ist je Tag idempotent
  (der 08:20-Retry postet nicht doppelt).
- **↩️ Kanal-Teil noch am selben Tag zurückgenommen** (Entscheidung Max):
  Die Zustellung bleibt wie gehabt — jeder Kunde bekommt die Morgenroutine in
  seinen eigenen Chat, Scanner-Signale gehen an Max. Siehe Eintrag darüber.
  **Der Scan-Zeitpunkt 10:00 ET bleibt bestehen.**
- **Betroffene Dateien:** `config/settings.py`, `bot/notifier.py`,
  `bot/morning_telegram.py`, `bot/morning_delivery.py`, `scheduler.py`,
  `web/app.py`, `web/templates/scanner.html`,
  `scripts/telegram_kanal_id.py` (neu), `tests/test_telegram_kanal.py` (neu),
  `CLAUDE.md`.
- **Verifiziert:** Suite **654 passed / 1 skipped / 0 failed**.
- **Status:** fertig, wartet auf die Kanal-ID
- **Für Sepp relevant:** ja — Signale und Morgenroutine erscheinen künftig
  öffentlich im Kanal, nicht mehr nur in privaten Chats.


### 2026-08-20 · Developer Agent · Scan-Fehler behoben + GC/CL/SI im Universum

- **Befund „Scan schlägt fehl":** Der Scanner selbst war in Ordnung — der
  05:42-Lauf ging sauber über alle 39 Symbole. Kaputt war das Drumherum: Der
  Knopf auf `/scanner` führte den Scan **im HTTP-Request** aus (Dauer ~3 Min),
  lief in den Timeout, und der Nachklick startete einen zweiten Lauf, der an
  der belegten IB-Client-ID scheiterte („Error 326: clientId already in use").
- **Behoben durch drei Dinge:**
  - Hintergrund-Thread statt blockiertem Request; die Seite zeigt ein
    Laufbanner und sperrt den Knopf, bis der Lauf fertig ist.
  - Prozessübergreifende Sperre (`journal/scan.lock`) mit Stale-/Tote-PID-
    Erkennung — ein zweiter Lauf wird im Klartext abgewiesen statt kryptisch.
  - Getrennte IB-Client-IDs: Scheduler 20, Handlauf 26.
- **Universum erweitert (Auftrag Max):** **GC, CL, SI** aufgenommen. CL stand
  bis dahin auf der Sperrliste des Regelwerks v2 („Energie ohne Ausnahme raus")
  und wurde auf ausdrückliche Ansage freigegeben; MCL/BZ/NG bleiben gesperrt.
- **Dabei mitgefunden und behoben:**
  - **Korrelationslücke:** GC neben MGC (und CL neben MCL) hätte denselben
    Rohstoff zweimal ins Portfolio bringen können — die Zählgrenzen sahen zwei
    verschiedene Symbole. Jetzt als Ausschlusspaare hinterlegt.
  - **SI ohne Trend:** Sein Front-Month lief in 7 Tagen aus und trug nur 57
    Tagesbars, MA200 war daraus grundsätzlich nicht berechenbar. Trend-Proxy
    für GC/CL/SI ergänzt (GC=F/CL=F/SI=F).
- **Live verifiziert:** Alle drei liefern jetzt benannte Funnel-Gründe —
  GC Gold-Momentum −8,8 % · CL IVR 39,4 (Gate 40, um 0,6 Punkte verfehlt) ·
  SI IVR 15,8.
- **Betroffene Dateien:** `config/settings.py`, `bot/scanner.py`, `web/app.py`,
  `web/templates/scanner.html`, `tests/test_scanner_v2.py`,
  `tests/test_scanner_sizing.py`, `tests/test_scanner_page.py`, `.gitignore`,
  `CLAUDE.md`.
- **Verifiziert:** Suite **619 passed / 0 failed**.
- **Status:** fertig
- **Für Sepp relevant:** ja — CL ist entgegen REGELWERK_STILLHALTER v2.0 wieder
  im Universum, auf ausdrückliche Ansage von Max.


### 2026-08-20 · Developer Agent · neuner-productions.de auf die Hausfarben umgestellt

- **Was geändert:** Erste Fassung verworfen (Auftrag Max). Die Seite folgt jetzt
  den Markenfarben — Grau dunkel `#575756`, Grau hell `#C6C6C6`, Rot `#E30613` —
  in hellem, sachlichem Layout statt des dunklen Tech-Looks. Alle weiteren Töne
  sind Abstufungen genau dieser drei Werte.
- **Erzählung:** Aus der Hörspielproduktion wurde Softwareentwicklung. Eigener
  Abschnitt „Analog → Digital"; die Notenmarke aus dem Logo ist Bildmarke und
  Hintergrundmotiv. Text deutlich gekürzt, Auftritt bewusst als Verwaltung der
  eigenen Produkte, nicht als Agentur.
- **Betroffene Dateien:** `sites/neuner-productions/*`
- **Status:** fertig, weiterhin nicht ausgerollt (DNS fehlt)
- **Für Sepp relevant:** nein — reine Außendarstellung der Dachmarke.

### 2026-08-20 · Developer Agent · Unternehmensseite neuner-productions.de (statisch)

- **Was geändert:** Landingpage für die Dachmarke Neuner Productions neu
  angelegt — Startseite plus Impressum und Datenschutzerklärung. Bewusst
  produktneutral aufgebaut: Daily Option ist der einzige Live-Eintrag, weitere
  Produkte lassen sich als zusätzliche Karte ergänzen, ohne die Seite umzubauen.
- **Wie umgesetzt:** Rein statisch unter `sites/neuner-productions/`, **kein
  Proxy auf die Flask-App** — die Seite hat keine Logik und keine Formulare, ein
  statischer Root ist die kleinste Angriffsfläche und läuft weiter, auch wenn
  `optionsbot-web.service` steht. Keine externen Ressourcen (keine Google Fonts,
  kein CDN), daher keine Drittübertragung der Besucher-IP und eine entsprechend
  kurze Datenschutzerklärung. nginx-Vorlage inkl. strenger CSP in
  `deploy/nginx-neuner-productions.conf`.
- **Produkttext Daily Option:** nach Marketing-Leitplanken formuliert — als
  Markteinschätzung beschrieben, kein Ergebnisversprechen, mit sichtbarem
  Hinweis „Informationsdienst, keine Anlageberatung" direkt auf der Karte.
- **Betroffene Dateien:** `sites/neuner-productions/*`,
  `deploy/nginx-neuner-productions.conf`
- **Status:** fertig, **noch nicht ausgerollt** — blockiert durch DNS: die
  Domain löst derzeit nicht auf, ohne A/AAAA-Record schlägt auch certbot fehl.
- **Für Sepp relevant:** ja — Dachmarken-Auftritt oberhalb der Produktdomains;
  betrifft weder Scanner noch Morgenroutine.

### 2026-08-20 · Developer Agent · Ausstiegsregelwerk vereinheitlicht (TP 50 / 7 DTE / Stop 2×)

- **Was geändert:** Auftrag Max — **Take-Profit 50 %, Zeit-Exit 7 DTE, Stop 2×
  Prämie**. Damit ist die Divergenz aufgelöst, die beim Scanner-Neubau am selben
  Tag entstanden war (Scanner plante 40 %/7 DTE, Überwachung meldete nach
  60 %/21 DTE).
- **Wie umgesetzt:** Der Satz steht jetzt **einmal** in `config/settings.py`
  (`EXIT_TAKE_PROFIT_PCT` / `EXIT_TIME_DTE` / `EXIT_STOP_LOSS_MULT`); Regelwerk,
  Strategy Engine, Scanner-Planung und `ASSET_CONFIG` leiten daraus ab. Ein
  erneutes Auseinanderlaufen ist strukturell ausgeschlossen und zusätzlich per
  Test festgehalten (`tests/test_exit_regelwerk.py`).
- **Folgen, bewusst in Kauf genommen:**
  - Zeit-Exit 21 → 7 DTE ist die folgenreichste Zahl: deutlich tiefer in der
    Gamma-Zone. Bei Short-Δ 0,30 und 45 DTE Laufzeit gewollt, beim alten
    Delta-Band 0,10–0,15 wäre es nicht vertretbar gewesen.
  - Staffelung FOP 21 / Aktien 14 DTE entfällt — ein Satz für alle Symbole.
  - Delivery-Exits (Energy 3 · Metalle/Agrar 5 · FX 2 DTE) werden vom 7-DTE-Exit
    überholt und greifen praktisch nicht mehr; sie bleiben als zweite Linie.
    Physische Lieferung ist dadurch früher abgesichert, nicht schlechter.
  - `backtest/` liest dieselben Konstanten und misst ab sofort die neuen Exits
    mit den alten Einstiegs-Parametern — Ergebnisse sind ein Hybrid und nicht
    mit Läufen vor dem 20.08. vergleichbar.
- **Auslegungsentscheidung:** Die Risiko-Backstops (Break-Even Prio 1,
  Quick-Win 25 %/7 T, Delta-Watchdog |Δ|>0,50) bleiben aktiv. Max hat drei
  Kern-Trigger benannt, die Backstops nicht abbestellt — sie stillzulegen hieße,
  Schutznetze für laufende Positionen ohne Auftrag zu entfernen.
  **Zu beachten: Quick-Win greift vor dem Take-Profit.**
- **Betroffene Dateien:** `config/settings.py`, `tests/test_exit_regelwerk.py`
  (neu), `tests/test_strategy_engine.py`, `tests/test_scanner_v2.py`,
  `tests/test_scanner_sizing.py`, `tests/test_rules.py`, `web/app.py`,
  `web/templates/scanner.html`, `CLAUDE.md`, `REGELWERK.md`, `TODO.md`.
- **Verifiziert:** Suite **601 passed / 0 failed**. Tests, die zuvor die alten
  Zahlen als Literal führten, lesen sie jetzt aus der Konfiguration — sie prüfen
  damit Verhalten statt Konstanten und überstehen die nächste Policy-Änderung.
- **Status:** fertig
- **Für Sepp relevant:** ja — die in `REGELWERK.md` dokumentierte Ausstiegsseite
  ist damit ebenfalls überholt; dort steht die Gegenüberstellung.


### 2026-08-20 · Developer Agent · Scanner v2 aktiviert (REGELWERK_STILLHALTER v2.0)

- **Was geändert:** Der am 17.08. zurückgebaute Scanner ist wieder aktiv — als
  **Neuentwicklung**, nicht als Wiederherstellung. Übernommen wurde nur die
  bewährte IB-Mechanik (autoritative Strikes, Chain-Merge, dreistufiger
  Kursabruf, Liquid-Hours-Gate); die Regelschicht ist vollständig neu.
- **Kernprinzip:** Die **Kontogröße** ist der einzige Eingangsparameter. Aus ihr
  folgen Risikobudget, Stückzahl (`floor(Budget / MaxLoss)`, 0 = kein Trade),
  Aktien-Kursobergrenze und Portfolio-Deckel. Für eine andere Kontogröße ist
  kein zweiter Scanner nötig. Instrumente schalten sich **emergent** frei —
  keine kodierten „ab 40k"-Schwellen.
- **Neue Parameter:** Short-Δ 0,30 (statt 0,125), Long-Δ 0,10 (MES 0,15),
  Spread-Breite **emergent** statt Prämienband, DTE 40–50, IVR ≥ 40,
  Regime-Staffelung 4/3/2 % nach VIX/IVR/MA200. Earnings blocken Aktien hart.
  MA200 ist kein Handelsverbot mehr, sondern bestimmt Richtung + Größe;
  RSI-70/30-Gate ersatzlos entfallen. Keine Verlust-Exits in der Planung.
- **Universum:** 5 Micro-Futures (MYM, MES, MGC, MNQ, M2K), **CL/MCL dauerhaft
  ausgeschlossen**. Aktien nicht mehr über eine Namensliste, sondern über die
  Kursobergrenze aus dem Konto (34er-Prüfkorb, bei 25k/4 % ⇒ ~106 USD).
- **Entscheidungen Max (19.08.):** `IVR_REGIME_HIGH` = 80 · Kontogröße **nur
  manuell**, kein Live-NLV · M2K mit RVX statt Ausschluss · breiter Aktienpool.
- **Zwei Befunde aus dem Live-Smoke (beide behoben):**
  - `^RVX` gibt es bei yfinance nicht („possibly delisted"). M2K bezieht seinen
    IVR jetzt über IB (`RVX@CBOE`, 251 Bars) — sonst wäre das Symbol dauerhaft
    fail-closed übersprungen gewesen.
  - MYM liefert nur **180** Tagesbars, MA200 ist aus dem Kontrakt also gar nicht
    berechenbar. Neuer Index-Proxy (`SCAN_TREND_PROXY_YF`), wobei Kurs **und**
    MA200 aus derselben Reihe stammen (sonst verfälscht der Carry die Richtung).
- **Betroffene Dateien:** `bot/scanner.py` (neu), `bot/sizing.py` (neu),
  `bot/scanner_config.py` (neu), `bot/market_data.py`, `bot/notifier.py`
  (`notify_signal`/`notify_no_candidates` neu), `config/settings.py`,
  `scheduler.py` (Scan-Job 09:35 ET), `main.py` (`scan`-Modus),
  `web/app.py` + `web/templates/scanner.html` (neu) + `base.html`,
  `tests/test_scanner_sizing.py` / `test_scanner_v2.py` / `test_scanner_page.py`
  (neu), `CLAUDE.md`, `REGELWERK.md`.
- **Verifiziert:** Suite **579 passed / 0 failed** (vorher 464). Live-Smoke gegen
  das Gateway: EURUSD live, VIX 14,89, Kursobergrenze 106,21 USD; alle fünf
  Micros laufen sauber durch und werden mit benanntem Grund verworfen
  (IVR im 1-Jahres-Tief, Gold-Momentum −8,6 %). Struktur- und Sizing-Stufen
  gegen echte Quotes geprüft: MES 7625P/7350P, Breite 275, Credit 37,25,
  MaxLoss 1.188,75 USD je Kontrakt gegen ein Budget von 1.168,36 USD ⇒
  **Stückzahl 0, kein Trade** — die Struktur wird nicht verengt.
- **Status:** fertig
- **Für Sepp relevant:** ja — Einstiegssignale sind wieder möglich; das
  Regelwerk der Einstiegsseite ist ein anderes als in `REGELWERK.md`
  (Gegenüberstellung dort im Gültigkeitsvermerk).
- **⚠️ Offener Punkt für Max:** Der Scanner *plant* TP 40 % / Zeit-Exit 7 DTE
  und weist beides im Signal aus. Die laufende Überwachung arbeitet unverändert
  nach 2026_01 (TP 60 %, Zeit-Exit 21 DTE, Stop 2×) — der Auftrag untersagte
  ausdrücklich, die Engine anzufassen. Solange beides nebeneinander steht, sind
  die v2-Exits von Hand einzuhalten.


### 2026-08-19 · Developer Agent · Mail-Absender auf dailyoption.de, Passwort-Leak behoben
- **Was geändert:** Der Versand läuft jetzt vollständig über die Kundendomain.
  `SMTP_USER`/`SMTP_PASSWORD`/`SMTP_FROM` = `morgenroutine@dailyoption.de`;
  IONOS bindet den erlaubten Absender an das angemeldete Postfach, deshalb
  mussten alle drei gemeinsam wechseln. Testmail zugestellt, alle drei
  Kundenadressen als Absender freigegeben.
- **Sicherheitsvorfall (selbst verursacht, behoben):** Das SMTP-Passwort des
  alten Postfachs stand im Klartext im systemd-Journal und in den sudo-Logs —
  Ursache war ein von mir eingeführtes Aufrufmuster
  (`systemd-run --setenv=SMTP_PASSWORD=…`). Alles auf der Kommandozeile landet
  im Journal und in der Prozessliste, letztere für jeden Systemnutzer lesbar.
  Umgestellt auf `-p EnvironmentFile=`; verifiziert, dass nichts mehr im
  Journal landet. **Offen bei Max: Passwort von `noreply@steadyalpha.de` bei
  IONOS wechseln** — nachträglich aus dem Journal entfernen ginge nur durch
  pauschales Löschen von Logs.
- **Betroffene Dateien:** `scripts/set_mail_sender.sh` (neu),
  `scripts/testmail.sh` (neu), `scripts/check_mail_sender.py`,
  `scripts/verify_domain_split.sh`, `/etc/optionsbot/secrets.env`
- **Status:** Domain-Trennung damit vollständig — Web, Mail und Telegram
  laufen unter `dailyoption.de` als „Daily Option".
- **Für Sepp relevant:** ja — ab morgen früh trägt die Morgenroutine den neuen
  Absender; Kundenkommunikation ist vollständig auf die neue Marke umgestellt.


### 2026-08-19 · Developer Agent · Domain-Trennung LIVE — Kundenseite auf dailyoption.de
- **Was geändert:** Teil A (nginx-Site + Zertifikat) ausgeführt und die
  Kundenseite scharf geschaltet. `CUSTOMER_BASE_URL=https://dailyoption.de`
  steht in `/etc/optionsbot/secrets.env`, beide Dienste neu gestartet.
  - nginx-Site `dailyoption` installiert (Apex kanonisch, www → 301 Apex),
    Let's-Encrypt-Zertifikat für `dailyoption.de` + `www.` gültig bis
    **2026-11-17**, Auto-Renewal über `certbot.timer` aktiv.
  - `/stripe/webhook` wird auf Apex **und** www direkt durchgereicht,
    verifiziert: 400 statt 301 (Stripe folgt keinem Redirect).
  - **Absender korrigiert:** `SMTP_FROM` trug noch den Anzeigenamen
    „Steady Alpha" — die *allererste* Marke. Anzeigename entfernt, damit ihn
    `bot/mailer.py` aus `PRODUCT_NAME` zieht und er künftig mitschaltet. Die
    Absenderadresse bleibt `@steadyalpha.de`, bis die neuen Postfächer
    existieren — ein Adresswechsel auf eine nicht angelegte Mailbox hätte den
    Versand gebrochen.
- **Verifiziert (Skript `scripts/verify_domain_split.sh`, alles grün):** DNS
  v4+v6, Zertifikatsabdeckung, HTTPS 200, Weiterleitungsketten je 1 Hop ohne
  Schleife, Webhook nicht umgeleitet, Admin-Domain erreichbar, MX/SPF/DMARC.
  Zusätzlich live geprüft: Admin-Pfade auf der Kundendomain → 301 auf
  steadyalpha, Kundenpfade auf der Admin-Domain → 301 auf dailyoption,
  Produktname je Domain korrekt, Impressum-Kontakt `info@dailyoption.de`,
  Testmail vom neuen Absender zugestellt, keine Fehler in den Dienstprotokollen.
- **Faktenlage geklärt:** `dailyoption.io` ist **nicht registriert**,
  `dailyoption.com` gehört einem Dritten. Kanonisch ist `dailyoption.de` —
  damit ist der `.io/.de`-Widerspruch des Auftrags entschieden.
- **Betroffene Dateien:** `/etc/nginx/sites-available/dailyoption` (Kopie in
  `deploy/`), `/etc/optionsbot/secrets.env` (Backup `.bak-domainsplit-20260819`),
  `deploy/AUFTRAG_THOMAS_domain-split.md`, `scripts/verify_domain_split.sh`
- **Status:** fertig. **Offen bei Max:** Stripe-Endpoint auf der neuen Domain
  (parallel zum alten anlegen → signierter Test → alten deaktivieren).
  **Offen bei Thomas:** IONOS-Postfächer `noreply@`/`morgenroutine@`/`info@`,
  DKIM, Mail-Warmup — erst danach wird `SMTP_FROM` auf die neue Adresse gezogen.
- **Für Sepp relevant:** ja — Kundenkommunikation läuft ab sofort unter
  `dailyoption.de` und dem Produktnamen „Daily Option"; steadyalpha.de ist
  reine Admin-Domain.


### 2026-08-19 · Developer Agent · Domain-Trennung Admin ↔ Kunde (Teil B, App-Seite)
- **Was geändert:** Eine App bedient künftig zwei Domains — `steadyalpha.de`
  (Max-intern: Scanner-Dashboard, `/admin/*`, Journal, Positionen, Logs) und
  die Kundendomain (Landing, Registrierung, Login, Abo, Morgenroutine, Stripe,
  Rechtstexte). Kein Fork, kein zweites Deployment: Host-Routing im
  `before_request` plus die nginx-Schicht aus Teil A.
  - **Ein Schalter, fail-closed:** Solange `CUSTOMER_BASE_URL` nicht in
    `/etc/optionsbot/secrets.env` steht, bleibt die Kundenseite vollständig auf
    steadyalpha.de — Mail-Links, Double-Opt-in, Stripe-Rückkehradressen,
    Impressum-Kontakt und Produktname schalten **gemeinsam** um. Eine halb
    migrierte Umgebung ist damit nicht möglich; Rückweg = Zeile entfernen.
  - Admin-Endpunkte werden von den Zugriffsdekoratoren **automatisch**
    registriert — eine künftig ergänzte Admin-Route kann nicht vergessen werden
    und damit nicht unter der Kundendomain erscheinen (fail-closed).
  - Stripe-Webhook wird unter **keinem** Host umgeleitet (Stripe folgt keinem
    301 → wäre ein stiller Zahlungsausfall); Checkout-Rückkehradressen kommen
    aus der Konfiguration statt aus dem Request-Host.
  - Produktname, Kontaktadresse und Domain stehen nur noch in
    `config/settings.py`; Templates und Mailtexte lesen sie.
  - Landing-Screenshot neu erzeugt: zeigte noch die **vorletzte** Marke
    („Steady Alpha") und `www.steadyalpha.de`.
- **Betroffene Dateien:** `config/settings.py`, `web/app.py`,
  `web/templates/*.html` (17), `bot/mailer.py`, `bot/morning_mail.py`,
  `bot/morning_telegram.py`, `bot/morning_payload.py`, `bot/morning_view.py`,
  `bot/morning_pdf.py`, `web/static/beispiel-morgenroutine-2026-08-14.png`,
  `tests/test_domain_split.py` (neu, 47 Tests)
- **Status:** fertig auf der App-Seite; **scharf geschaltet wird erst**, wenn
  Teil A (DNS, Zertifikat, IONOS-Mail) verifiziert ist — Reihenfolge in
  `TODO.md`. Testsuite 461 grün.
- **Entscheidungen (autonom nach CLAUDE.md, weil Auftrag widersprüchlich bzw.
  offen):**
  1. **Kundendomain `.de`** — der Auftrag nennt in Überschrift und DNS-Schritt
     `.io`, in allen operativen Teil-B-Punkten dagegen `.de` (BASE_URL,
     Postfach, nginx-Site, IONOS-Mail). Umgesetzt ist `.de`; `.io` und `.com`
     werden als Alias bedient und per 301 auf die kanonische Domain geführt.
     Umschalten = Wert einer Variablen, kein Code.
  2. **Produktname „Daily Option"** (Option A des Auftrags, dort empfohlen).
     Hängt an einer Konstanten und schaltet gemeinsam mit der Domain um, damit
     kein Zwischenzustand „Daily Option auf steadyalpha.de" entsteht.
- **Für Sepp relevant:** ja — Kundenkommunikation läuft künftig unter neuer
  Domain und neuem Produktnamen; beide Entscheidungen sind reversibel, aber
  der Name gehört laut Auftrag in den Anwalts-Review-Scope.


### 2026-08-18 · Developer Agent · Gateway-Erholung wird sofort erkannt

Beim Wiederanlauf heute aufgefallen: Der Verbindungs-Gate prüfte im DOWN-Zustand
60 Minuten lang gar nicht mehr nach. Das Gateway lief längst wieder, während
Strategy-Engine und IB-Sync weiter übersprungen wurden — bis zu 12 unnötig
ausgelassene Überwachungszyklen. Die Drosselung war gegen Telegram-Flut gedacht,
die aber ohnehin separat geregelt ist; die Prüfung selbst ist ein
TCP-Verbindungstest auf dem eigenen Rechner und kostet nichts.

- Jetzt: jeder Zyklus prüft, gedrosselt bleibt nur der Alarm. Eine Erholung
  wirkt damit binnen 5 Minuten statt bis zu 60.
- `CONNECTION_RETRY_INTERVAL_MIN` steuerte danach nichts mehr → entfernt.
  Ein Parameter, der still nichts tut, war heute schon einmal die Fehlerursache.
- Betroffene Dateien: `bot/ib_gate.py`, `config/settings.py`, `tests/test_ib_gate.py`
- Getestet: 414 Tests grün; live verifiziert (DOWN → CONNECTED, Recovery-Meldung,
  Engine-Zyklus mit offenem Spread durchgelaufen)
- Status: fertig
- Für Sepp relevant: ja — Ausfallzeiten der Positionsüberwachung verkürzen sich
  spürbar


### 2026-08-18 · Developer Agent · Gateway-Watchdog wieder funktionsfähig + Wartungsfenster

**Befund:** Das IB Gateway war seit 18:19 Uhr tot — Max hatte es bewusst über die
Web-UI gestoppt (Login in der IBKR-App, IB erlaubt nur eine Sitzung). Der
Watchdog hätte es danach automatisch hochfahren müssen und konnte es nicht: Die
Neustarts scheiterten am fehlenden Rechteweg des Scheduler-Dienstes. Folge: über
vier Stunden keine Überwachung der offenen Positionen, ohne dass es auffiel.

**Zwei Änderungen:**
1. **Rechteweg:** `_systemctl()` versucht jetzt zuerst den direkten Weg und
   fällt auf den alten zurück. Ein nicht absetzbares Start-Kommando bricht
   sofort ab, statt 240 s ins Leere zu pollen. Die nötige Systemregel liegt
   versioniert unter `deploy/` — **Installation durch Max nötig** (siehe unten).
2. **Wartungsfenster (neu):** Ein gewollter Stopp über die Web-UI hält den
   Watchdog `GATEWAY_PAUSE_MINUTES` (90) Minuten lang zurück; der manuelle Start
   beendet es sofort. Ohne das hätte der reparierte Watchdog Max binnen Minuten
   wieder aus der IBKR-App geworfen — die Reparatur hätte ein neues Problem
   erzeugt.

- Betroffene Dateien: `bot/gateway_watchdog.py`, `bot/ib_gate.py`, `web/app.py`,
  `config/settings.py`, `deploy/`, `tests/test_gateway_pause.py`
- Getestet: 413 Tests grün (vorher 405 + 1 rot). Der rote Test
  (`test_ensure_gateway_fast_path_no_restart`) las den Produktivzustand mit und
  schlug deshalb bei echtem Gateway-Ausfall fehl — jetzt isoliert.
- Status: teilweise — Gateway läuft seit 18.08. 22:46 wieder (Max hat die
  Zwei-Faktor-Freigabe erteilt), Überwachung verifiziert: Engine verbunden,
  offener Spread bewertet. **Offen:** der Rechteweg braucht noch einen
  Installationsschritt als root; bis dahin kann der Watchdog einen erneuten
  Ausfall weiterhin nicht selbst beheben.
- Für Sepp relevant: ja — die Positionsüberwachung war faktisch ohne
  Selbstheilung; das Wartungsfenster ist neues erwartbares Verhalten der UI.


### 2026-08-18 · Developer Agent · Morgenroutine-Ausfall 18.08. behoben (drei Ursachen)

**Befund:** Am 18.08. ging keine Morgenmail raus. Die Routine lief und speicherte
korrekt, aber das DD-Publikations-Gate schlug fail-closed fehl → kein PDF → kein
Versand (Heartbeat 08:35: „Morgenmail fehlt bei 1 von 1 Empfängern"). Ursache war
kein Rechen-, sondern ein Abruf-Fehler: yfinance lieferte um 07:45 für ^GSPC **und**
^IXIC gleichzeitig `Close = NaN` für den 17.08. Dieselben Daten waren später
einwandfrei — ein transienter Yahoo-Aussetzer legte damit den ganzen Kundentag lahm.

**Drei Fehler, alle behoben:**
1. **Alarm kam nie an.** `notifier.send()` sendet mit `parse_mode=HTML`; die
   Fehlermeldung enthält „Close fehlt oder <= 0" — Telegram las `<=` als Start-Tag
   und antwortete 400. Ausgerechnet die Alarme, die Max erreichen müssen, gingen
   verloren. Jetzt Klartext-Fallback bei 400 (reproduziert und verifiziert).
2. **Kein Neuversuch beim Abruf.** `_fetch_dd_history()` wiederholt den Abruf bis
   zu 3× (4 s Pause), wenn das Gate-Fenster NaN enthält. Bleiben die NaN, greift
   das Gate unverändert fail-closed.
3. **Retry 08:20 war wirkungslos.** Er las nur das bereits fehlerhafte Archiv neu.
   Bei gescheitertem Gate wird jetzt zuerst der Datenabruf wiederholt, dann
   zugestellt — ein transienter Aussetzer kostet damit 35 Minuten statt den Tag.

- Betroffene Dateien: `bot/notifier.py`, `bot/morning_routine.py`, `scheduler.py`
- Getestet: DD-Gate mit Live-Daten PASS (ES 3 / NQ 3), Telegram-Fallback zugestellt,
  Gate-Erkennung am kaputten Archiv, 405 Tests grün
- Status: fertig — greift ab dem Lauf 19.08. 07:45
- Nicht nachversandt: die Ausgabe vom 18.08. wurde bewusst **nicht** abends
  nachgeschickt (Datenstand überholt, Zustellung an Kunden um 22:15 Uhr)
- Für Sepp relevant: ja — ein stiller Totalausfall der Kundenzustellung inkl.
  nicht zustellbarem Alarm; die fail-closed-Kette funktionierte, hatte aber keinen
  Weg zurück aus einem transienten Datenfehler
- Offen (nicht Teil dieses Fixes): `tests/test_gateway_watchdog.py::
  test_ensure_gateway_fast_path_no_restart` schlägt schon auf HEAD fehl
  (Test simuliert das Gateway mit einem nackten Socket, `ensure_gateway` prüft
  inzwischen mehr) — separat anzusehen, ebenso das seit 17.08. dauerhaft
  nicht erreichbare IB Gateway.


### 2026-08-17 · Developer Agent · Scanner v1 vollständig zurückgebaut (Neuentwicklung folgt)
- Was geändert (Auftrag Max: „Entferne komplett den aktuellen Scanner, räume alles auf,
  ich entwickle ihn später mit neuen Regeln neu"). **Gelöscht:** `bot/scanner.py` (1.411 Z.),
  `check_entry()` + `SpreadCandidate` (`bot/rules.py`), `notify_signal()` +
  `notify_no_candidates()` (`bot/notifier.py`), Aufgabe 1 der Strategy Engine
  (`build_entry_ticket`/`emit_entry_tickets`/`_emit_ticket` — hatte nach dem Rückbau keine
  Datenquelle mehr), Scan-Job 09:35 ET (`scheduler.py`), CLI-Modus `main.py scan` + `--notify`,
  Web-Route `/setup` + Template + Navigationseintrag + Aktion `run_scan` + Scan-Buttons,
  Scan-Zeit im Status-Strip, `_load_scan_results()`, sowie **alle Einstiegs-Parameter aus
  `ASSET_CONFIG`** (delta, delta_band, dte_min/max/target, ivr_min, max_spread_width,
  spread_efficiency_min, liquidity_ba_max_pct, min_oi_*, theta_min_pct, earnings_check,
  bcs_*) und das Prämienband (`PREMIUM_TARGET_USD_*`, `PREMIUM_QTY_MAX`,
  `MAX_PROPOSALS_PER_SYMBOL`, `EURUSD_STATIC`, `SCAN_RESULTS_FILE`, `BATCH_SIZE`).
- **Neu:** `bot/market_data.py` — die regelwerksfreien Marktdaten-Funktionen aus dem Scanner
  (`calc_iv_metrics`, `get_front_months`, `is_future`, `parse_expiration`, `rank_pct`,
  `calc_index_iv_rank`, `calc_ib_index_iv_rank`, Vola-Index-Registries). Grund: Die Strategy
  Engine braucht sie für den IVR-Kollaps-Alert der **2 offenen Positionen** (CL 70/68P exp
  17.09., GC 4080/4030P exp 24.09.) — ein reiner Scanner-Löschlauf hätte deren Überwachung
  gebrochen. Tests mitgezogen: `test_scanner_ivr.py` → `test_market_data_ivr.py` (9 grün).
- **Bewusst NICHT angetastet** (Überwachung offener Positionen + Kundenprodukt):
  `check_exit()`, `bot/manager.py`, Strategy Engine Aufgaben 2+3, `bot/ib_sync.py`,
  komplette Morgenroutine samt Zustellung, Marktlage/Sektor-Lage im Dashboard
  (Quelle ist die Morgenroutine, nicht der Scanner).
- **Daten archiviert statt gelöscht:** `journal/scans/` (18 Läufe, 114 Kandidaten, Funnel) +
  `scan_results.json` + `last_scan.txt` → `docs/archiv/scanner_v1/` mit README. Rückwirkend
  nicht neu erzeugbar (historische Optionsketten sind weder über IB noch yfinance abrufbar)
  und Grundlage für die Kalibrierung des neuen Regelwerks.
- **Verifiziert:** Suite **405 passed**, 1 failed — `test_gateway_watchdog.py::
  test_ensure_gateway_fast_path_no_restart` war **schon vor dem Rückbau rot** (Baseline
  446/1 vor, 405/1 nach; Differenz = entfernte Scanner-Tests) und ist nicht angefasst.
  Zusätzlich: alle Web-Seiten rendern (Gast + Admin, `/setup` sauber 404), Scheduler
  registriert nur noch gateway_prewarm/monitor/ib_sync/pcr_sample/strategy_engine/
  Dependency-Scan/Morgenroutine-Jobs, und `check_exit` wurde funktional gegen beide offenen
  Positionen geprüft (PROFIT_TARGET / STOP_LOSS / BREAK_EVEN / kein-Exit greifen korrekt).
- Betroffene Dateien: `bot/scanner.py` (gelöscht), `bot/market_data.py` (neu), `bot/rules.py`,
  `bot/notifier.py`, `bot/strategy_engine.py`, `config/settings.py`, `main.py`, `scheduler.py`,
  `web/app.py`, `web/templates/base.html`, `web/templates/dashboard.html`,
  `web/templates/setup.html` (gelöscht), `tests/*`, `CLAUDE.md`, `REGELWERK.md`
- Status: fertig — Scanner-Neuentwicklung offen (eigener Auftrag Max)
- Für Sepp relevant: **ja** — es gibt derzeit **keinen Scanner und keine Einstiegssignale**.
  Der Bot überwacht nur noch offene Positionen und liefert die Morgenroutine. Einstiegsregeln
  im alten Regelwerk (Delta-Band, IVR-Gate, DTE-Fenster, Effizienz-Gate, Prämienband) sind
  **nicht mehr im Code** — Aussagen dazu gelten bis zur Neuentwicklung als historisch.

### 2026-08-17 · Developer Agent · E-Mail und Telegram im Satz des PDF
- **Auftrag Max:** „Das PDF gefällt — in diesem Stil bleiben." E-Mail und
  Telegram wurden auf dieselbe Anmutung gezogen; welche Inhalte in welchen
  Kanal gehören, war meine Entscheidung (dokumentiert unten).
- **Neue gemeinsame Ansichtsschicht `bot/morning_view.py`:** Alle drei
  Kundenkanäle bauen ab sofort aus **einem** Payload (`morning_payload`) und
  einer Formatierung. Vorher zog jeder Kanal seine Zahlen selbst aus dem
  Routine-Dict — Telegram zeigte die Marktbreite als „NYSE-A/D 52 %", das PDF
  dieselbe Lage als „1,12 : 1". Solche Widersprüche sind jetzt strukturell
  ausgeschlossen. Die PDF-Palette liegt dort als Single Source für Kanäle, die
  kein CSS laden können.
- **E-Mail:** Kopfleiste mit Bildmarke, Zustandsfeld mit dreilampiger Ampel,
  Karten mit Zonenkante — gebaut mit Tabellen/Inline-Styles (Outlook-fest),
  Ampeln als Zeichen statt Bild (Bildblocker). **Inhaltsschnitt:** Die Mail
  zeigt Zustand und Zahlen (Volatilität, Terminkurve/Sentiment, Marktbreite,
  Verteilung, Panik-Checks) — genug, um am Handy zu entscheiden, ob der Tag
  Aufmerksamkeit braucht. Die **Einordnung** („Was die Werte bedeuten"), die
  Veränderungstabelle und die MA21-Übersicht bleiben dem PDF vorbehalten: Es
  behält damit seinen Anlass, geöffnet zu werden. Die Kurzfassung von 08-10
  lebt als Rückfallebene weiter (kein Payload → schmales Anschreiben, aber
  Zustellung).
- **Telegram:** Deko-Emojis raus (📊🎭📈🗓), Ampel-Emojis bleiben — im Chat der
  einzige Farbkanal. Feste Zustellfolge: Bild + Kurz-Caption (Zustand, Fazit,
  vier Dimensionen) → Kennzahlen-Nachricht (Blöcke der Seite 1 + Pflichtangaben)
  → PDF. Vorher hing die Aufteilung an der Zufallslänge der Caption.
- **Telegram-Vorschaubild neu gezeichnet** in der PDF-Palette: Kopfleiste,
  Zustandsfeld, sechs Kacheln (VIX, VVIX, Terminkurve, Fear & Greed, Put/Call,
  Verteilung), Marktbreite/Panik. Die beiden Tachos kommen aus `bot/gauge.py` —
  derselben Quelle wie Dashboard und PDF. Bewusst kompakt statt „PDF-Seite als
  Bild": in der Chat-Liste wird auf ~400 px skaliert, eine A4-Seite wäre dort
  unlesbar.
- **`scripts/install_fonts.py`** (neu): installiert Inter/Space Grotesk/
  JetBrains Mono als TTF systemweit, damit der Bild-Renderer (cairosvg kann
  keine woff2) die Hausschrift benutzt. Auf dem VPS ausgeführt; ohne den
  Schritt fällt nur das Bild auf DejaVu zurück, nichts bricht.
- **Betroffene Dateien:** `bot/morning_view.py` (neu), `bot/morning_mail.py`,
  `bot/morning_telegram.py`, `scripts/install_fonts.py` (neu),
  `tests/test_morning_view.py` (neu), `tests/test_morning_delivery.py`
- **Tests:** 447 grün. Neu abgesichert: identische Zahlen in allen Kanälen,
  deutsche Zahlformatierung, „in Prüfung" statt Zahl ohne DD-Gate, Escaping von
  Fremdtext, Telegram-HTML nur mit von der API erlaubten Tags, Caption-Limit,
  Pflichtangaben in jedem Kanal, Sprachregelung (keine Strategienamen).
- **Status:** fertig, Dienste neu gestartet
- **Für Sepp relevant:** ja — der Inhaltsschnitt Mail ↔ PDF ist neu und sollte
  in der Produktbeschreibung so vertreten werden.

### 2026-08-14 · Developer Agent · Umbenennung Steady Alpha → Steady Ground (ohne Domain)
- **Entscheidung Max.** Nebeneffekt: Der UWG-Befund zum alten Namen („Alpha" =
  Anmutung von Überrendite) erledigt sich damit.
- **Umfang:** 54 Dateien — alle Templates, Rechtstexte, PDF-Kopf und -Fuß,
  E-Mail, Telegram, Betreffzeilen, Doku, Projekt-Skills. Die Skill-Verzeichnisse
  heißen jetzt `steady-ground-{marketing,ui,security}`.
- **Domain bleibt vorerst `steadyalpha.de`** (Umstellung durch Max). Ebenfalls
  unverändert, weil extern: der Telegram-Bot-Username und der nginx-Konfigname.
- **Dateinamen migriert:** Das Tagesartefakt heißt `steady-ground-morgenroutine-
  YYYY-MM-DD.pdf`; die sieben vorhandenen Archivtage wurden umbenannt. Ein
  Lesefallback auf den alten Namen bleibt bestehen, damit ein Restore aus einem
  älteren Backup nicht still ins Leere läuft. Auch die Backup-Rotation räumt
  weiterhin Archive unter dem alten Namen ab.
- **Layout-Befund nebenbei gefunden und behoben:** Sehr lange E-Mail-Adressen
  brachen in der neuen Empfängerzeile um, wodurch das PDF auf drei Seiten lief —
  das hätte den Versand fail-closed gestoppt. Adressen werden jetzt mittig
  gekürzt (Anfang und Domain bleiben lesbar), zusätzlich erzwingt das
  Stylesheet eine einzeilige Darstellung.
- **Bestandsausgaben:** Ältere Archiv-PDFs tragen im Dokument weiterhin den
  alten Namen — sie wurden unter diesem ausgegeben, das ist historisch korrekt
  und wird nicht rückwirkend geändert. Die Ausgabe vom 14.08. ist neu gerendert.
- Tests: 429 grün (1 neuer zur Adresskürzung). Live nach Neustart verifiziert.
- Status: fertig und live.
- Für Sepp relevant: ja — ab sofort in allen Texten „Steady Ground"; die Domain
  wird separat umgestellt, bis dahin bleiben Links und E-Mail-Adressen alt.

### 2026-08-14 · Developer Agent · PDF trägt Empfängerkennzeichnung („Ausgabe für: …")
- **Wunsch Max:** Unter dem Kopf („Marktlage · Morgenroutine") steht jetzt
  `Ausgabe für: <E-Mail>` — Abschreckung vor Weitergabe, weitergereichte Kopien
  sind zuordenbar.
- **Umsetzung:** Das archivierte Tagesartefakt bleibt **neutral und personenfrei**;
  die Empfängerzeile entsteht erst beim Ausliefern im Speicher, nie auf Platte.
  Damit keine personalisierten Dateien mit Löschpflichten und keine zweite
  Wahrheit neben dem geprüften Archiv.
- Greift auf **allen drei Wegen**: E-Mail-Anhang, Telegram-Dokument und
  Portal-Download. Ohne alle drei wäre die Kennzeichnung mit einem Klick
  umgangen. Admin-Abruf bekannt neutral (Archivkontrolle).
- **Abgrenzung gewahrt:** Der Inhalt ist für alle Empfänger identisch — der
  Risikohinweis sagt das jetzt ausdrücklich, damit die Kennzeichnung nicht als
  individuelle Ausarbeitung missverstanden wird (Merkmal 4 des
  Anlageberatungs-Tests bleibt damit klar unerfüllt).
- **Fail-soft:** Scheitert das Stempeln, geht die geprüfte neutrale Fassung
  raus — ein fehlender Empfängerhinweis ist kein Grund, dem Kunden das Produkt
  vorzuenthalten.
- **Kosten/Skalierung (gemessen):** ~4 s Rendering je Empfänger und Kanal.
  Unkritisch bis ca. 100 Kunden (Versand ab 08:00 dann ~7 Min, Retry-Fenster
  08:20 bleibt frei). Darüber lohnt ein Overlay-Verfahren statt Vollrendering —
  vorgemerkt, heute nicht nötig.
- Betroffene Dateien: `bot/morning_pdf.py` (`issue_payload`/`render_issue`/
  `issue_for`), `bot/morning_payload.py`, `bot/morning_mail.py`,
  `bot/morning_telegram.py`, `web/app.py`, `web/templates/morning_pdf_macros.html`,
  `web/static/css/morning_pdf.css`, `docs/morning_payload.*`, `tests/test_morning_pdf.py`
- Tests: 6 neue (Zeile nur mit Empfänger, kein Seiteneffekt auf den
  wiederverwendeten Payload, identischer Inhalt für alle, zwei Seiten auch bei
  langer Adresse, Archiv personenfrei). Suite: 428 grün.
- Status: fertig und live; Archiv-PDF vom 14.08. neu gerendert.
- Für Sepp relevant: ja — die Empfängerzeile darf nicht zu inhaltlicher
  Personalisierung ausgebaut werden, sonst kippt die Abgrenzung.

### 2026-08-14 · Developer Agent · Rechtliche Vorprüfung umgesetzt (Kündigungsseite, MAR-Pflichtangaben, AGB)
- **Anlass:** Vorprüfung der kundenseitigen Artefakte gegen WpHG/WpIG (Anlageberatung),
  MAR Art. 20, UWG, Fernabsatzrecht, DDG/DSGVO. Ergebnis: Das Angebot bleibt außerhalb
  der Erlaubnispflicht (keine Personalisierung, keine Geeignetheitsprüfung, keine
  Ordervermittlung) — fünf Punkte waren aber nachzubessern.
- **Kündigungsschaltfläche § 312k BGB (war der schärfste Punkt):** Neue Seite
  `/kuendigen`, **ohne Anmeldung** erreichbar und aus allen Footern verlinkt.
  Zweistufig (Angaben → Bestätigungsseite → „Jetzt kündigen"), Bestätigung in
  Textform mit Zugangszeitpunkt und Vertragsende, Erklärung wird protokolliert.
  Vorher lag der Button nur hinter dem Login und war einstufig.
- **Bestätigung fail-closed:** Scheitert der Mailversand, gibt es Alarm statt
  stillem Ausfall; die Erklärung ist vorher protokolliert und geht nie verloren.
- **MAR-Pflichtangaben** in Web-Ansicht, PDF, E-Mail und Telegram ergänzt:
  Ersteller namentlich, Hinweis auf fehlende BaFin-Aufsicht, Verweis auf die
  Methodikseite als Bewertungsgrundlage, Zeithorizont — und erstmals eine
  **Offenlegung des Interessenkonflikts** (eigener Optionshandel auf dargestellte
  Basiswerte). Stand vorher: nirgends vorhanden.
- **Underlying-Tabelle entschärft:** „Prämien-Niveau" nicht mehr grün/gelb/rot,
  sondern monochrome Intensität; Tooltip beschreibt den Zustand statt Eignung.
  Ampelfarben je benanntem Basiswert lasen sich als Empfehlung.
- **Rechtstexte:** AGB § 2 (Interessenkonflikt), § 6 (Phasenabgrenzung +
  Kündigungswege), § 8 (Haftung klargestellt), § 10 (Änderungsklausel auf
  triftige Gründe begrenzt, Preis/Hauptleistung ausgenommen, Sonderkündigungs-
  recht); Widerrufsbelehrung korrigiert (beim laufenden Abo erlischt das
  Widerrufsrecht nicht) inkl. Alternativweg „Start erst nach Fristende".
- **Datenschutz:** Rechtsgrundlage der Telegram-Drittlandsübermittlung ergänzt
  (Art. 49 Abs. 1 lit. a DSGVO) und die Einwilligung bei Registrierung und im
  Profil informiert gestaltet.
- **Landing/Marketing:** H1 „die sich selbst rechnet" → „die sich selbst
  erledigt" (las sich als Ertragsversprechen), USt-Angabe im Preisblock,
  Interessenkonflikt im Risikohinweis.
- **Neu:** `/partnerbedingungen` — verbindliche Werberegeln für
  Empfehlungspartner (keine Renditeaussagen, keine Erfolgsquoten,
  Werbekennzeichnung); Partneraussagen sind uns zurechenbar.
- **Tests:** 8 neue Tests zur Kündigungsstrecke (Erreichbarkeit ohne Login,
  Zweistufigkeit, Textform-Bestätigung, Protokollierung, keine Preisgabe von
  Kundenkonten, serverseitige Prüfung). Suite: 422 grün. PDF hält weiterhin
  exakt zwei Seiten trotz längerem Pflichttext (Schema-Limit angehoben).
- Betroffene Dateien: `web/app.py`, `web/templates/kuendigen.html` (neu),
  `web/templates/partnerbedingungen.html` (neu), `agb.html`, `widerruf.html`,
  `impressum.html`, `datenschutz.html`, `landing.html`, `abo.html`,
  `register.html`, `profile.html`, `morning.html`, `base.html`,
  `legal_base.html`, `admin_referrals.html`, `bot/morning_payload.py`,
  `bot/morning_mail.py`, `bot/morning_telegram.py`, `docs/morning_payload.*`,
  `tests/test_kundenstrecke.py`
- Status: fertig und **live** — Freigabe durch Max am 14.08., Dienste neu
  gestartet, alle öffentlichen Seiten mit 200 verifiziert. Der Neustart lag
  nach dem Tagesversand (08:00 Berlin), die Zustellung war unberührt.
  Hinweis: Das PDF vom 14.08. war zum Freigabezeitpunkt bereits gerendert und
  trägt noch den alten Risikohinweis; ab dem 17.08. enthält jedes PDF die
  Pflichtangaben. Die anwaltliche Endfreigabe vor dem ersten zahlenden Kunden
  bleibt davon unberührt und ist weiterhin offen.
- Für Sepp relevant: ja — die Positionierung „Markteinschätzung, keine
  Empfehlung" ist jetzt auch in den Pflichtangaben verankert; Marketing-Texte
  dürfen die Interessenkonflikt-Zeile nicht wieder entfernen.

### 2026-08-13 · Developer Agent · Öffentlicher Mirror aufgeräumt (Befund: nicht der Mirror war alt)
- Ausgangsverdacht (Max): Der öffentliche Spiegel steht auf 2026-07-08 und ist
  damit älter als das Projekt. **Diagnose korrigiert das teilweise:** Die Action
  läuft, der letzte Sync war 13.08. 20:00 UTC, `STATUS.md` und `TODO.md` waren
  dort inhaltlich aktuell. Drei echte Ursachen für den Eindruck:
  1. **Falscher Repo-Name in allen Dokumenten.** Das Repo heißt tatsächlich
     `maxomantrader-de/optinsbot-state` (ohne zweites „o", Tippfehler bei der
     Anlage am 08.07.). `CLAUDE.md`, `STATUS.md` und der Mirror-README nannten
     `optionsbot-state` — diese Adresse liefert 404. Wer der Dokumentation
     folgt, findet den Spiegel nicht.
  2. **Kein sichtbarer Frischestand.** Auf der Repo-Startseite war das einzige
     Datum das **Anlagedatum 08.07.2026** — genau der berichtete Stand.
  3. **`TASKS.md` war tatsächlich veraltet** (2026-05-06, 676 Zeilen): eine
     komplett abgearbeitete Auftragsserie, inklusive vollständigem Quelltext
     für einen **Order-Executor** und der längst abgelösten `SEPP_QUEUE.md`-
     Bridge. Beides öffentlich sichtbar und im Widerspruch zum heutigen System
     (kein automatischer Handel, HTTP-Bridge über `sepp_tasks/queue/`).
- Was geändert:
  - `TASKS.md` neu geschrieben als schlanke **Auftragsschnittstelle**:
    Rangfolge der Dokumente, der real betriebene Sepp-Weg, ein offener Auftrag
    (Domain-Migration, liegt bei Max), Archivtabellen der erledigten Serien A
    (05/2026) und B (07–08/2026) mit Verweis auf die Auftragsdokumente.
  - Alter Volltext nach `docs/archiv/TASKS_ARCHIVIERT_2026-05-06.md` verschoben
    (nicht mehr gespiegelt, da die Whitelist nur die Repo-Wurzel kennt), im
    Archiv-README mit Begründung eingeordnet.
  - Mirror-README wird jetzt **mit Frischestand generiert**: Spiegelzeitpunkt,
    Quellstand (Datum + Kurz-SHA), letzte Änderung, Dateiübersicht und der
    ausdrückliche Hinweis, dass das Anlagedatum nichts über Aktualität sagt.
  - Repo-Adresse in `CLAUDE.md` und `STATUS.md` auf die reale Schreibweise
    korrigiert; Fokus-Datum und offene Max-Punkte nachgezogen.
- Betroffene Dateien: `TASKS.md`, `docs/archiv/TASKS_ARCHIVIERT_2026-05-06.md`,
  `docs/archiv/README.md`, `.github/workflows/mirror-state.yml`, `CLAUDE.md`,
  `STATUS.md`, `TODO.md`
- Bewusst **nicht** gemacht: das öffentliche Repo umbenennen. Ein Rename ist
  außenwirksam und ändert die URL — Entscheidung Max, als Punkt in `TODO.md`.
- Status: fertig
- Für Sepp relevant: **ja** — Einstiegspunkt ist
  `github.com/maxomantrader-de/optinsbot-state` (ohne zweites „o"). `TODO.md`
  ist die einzige Liste offener Punkte, `TASKS.md` nur noch Index und Archiv.


### 2026-08-13 · Developer Agent · Jahresabo: Variante A umgesetzt (fair verlängern)
- Entscheidung Max: Nach der Erstlaufzeit **keine zweite gebundene
  Jahresperiode**, sondern unbefristete Fortsetzung mit monatlicher
  Abrechnung zum anteiligen Jahrespreis (39,20 € statt 49 € Liste — der
  Rabatt bleibt), ab dann monatlich kündbar. Damit ist die Konstruktion aus
  § 309 Nr. 9 BGB heraus, ohne Erstattungslogik (Variante B verworfen).
- Was geändert: Nach dem Checkout richtet `_schedule_yearly_followup()` einen
  Stripe Subscription Schedule ein (Phase 1 = gebuchte 12 Monate unverändert,
  Phase 2 = ein Monat zum Zwölftelpreis, danach `release` → läuft monatlich
  weiter). Idempotent, bei Fehlschlag Telegram-Alarm + Event-Retry
  (fail-closed: ohne Folgephase würde erneut der Jahresbetrag abgebucht).
  `customer.subscription.updated` zieht Plan und Preis-Snapshot beim
  Phasenwechsel nach. Vorab-Mail kündigt jetzt die Umstellung samt neuem
  Monatsbetrag an. Pflichtangaben (§ 312j Abs. 2) nennen Erstlaufzeit **und**
  Folgepreis; Abo-Karte zeigt „Erstlaufzeit bis …" plus Folgepreis;
  AGB § 5/§ 6 entsprechend gefasst; Landingpage angepasst.
- Betroffene Dateien: `web/app.py`, `web/templates/abo.html`,
  `web/templates/agb.html`, `web/templates/landing.html`,
  `tests/test_kundenstrecke.py`
- Status: fertig · Tests 414 grün (4 neue) · **offen bei Max:** Event
  `invoice.upcoming` im Webhook aktivieren; E2E-Test im Stripe-Testmodus,
  sobald die PriceID steht (Phasenwechsel ist lokal getestet, aber noch nie
  gegen die echte Stripe-API gelaufen)
- Für Sepp relevant: ja — Preismodell des Jahresabos ist jetzt „12 Monate,
  danach monatlich zum Jahrespreis-Niveau"; Rechtstexte (AGB § 5/6) geändert
  und warten auf Freigabe durch Max.


### 2026-08-13 · Developer Agent · Automatische Verlängerung (Jahres- und Monatsabo)
- Was geändert: Stripe verlängert beide Abos technisch schon immer selbst, und
  AGB § 6 beschreibt einen Vertrag auf unbestimmte Zeit — nur die Abo-Seite
  behauptete „verlängert sich nur bei aktiver Buchung". Widerspruch aufgelöst
  zugunsten der Verlängerung: Pflichtangaben über dem Bestellbutton (§ 312j
  Abs. 2) nennen jetzt „12 Monate, verlängert sich automatisch um je 12 Monate"
  (analog Monatsabo), Landingpage-Plannotizen ebenso. AGB § 6 um einen
  expliziten Verlängerungsabsatz ergänzt (Nummerierung 2→3, 3→4).
  Abo-Karte zeigt den Verlängerungstag (ohne das interne Stripe-Kulanzfenster)
  plus Hinweis. Neu: `invoice.upcoming` → Erinnerungsmail vor der
  Jahresverlängerung mit Betrag und Kündigungsweg (Monatsabo bewusst nicht).
- Betroffene Dateien: `web/app.py`, `web/templates/abo.html`,
  `web/templates/agb.html`, `web/templates/landing.html`,
  `tests/test_kundenstrecke.py`
- Status: fertig · Tests 410 grün (6 neue) · offen bei Max: Event
  `invoice.upcoming` im Stripe-Dashboard-Webhook aktivieren (Vorlauf einstellbar)
- Für Sepp relevant: ja — Preis-/Laufzeitkommunikation ist jetzt überall
  identisch. Rechtlicher Hinweis für Max/Sepp: die Verlängerung um weitere
  12 Monate ist bei Verbrauchern die von § 309 Nr. 9 BGB (Fassung seit
  03/2022) diskutierte Konstruktion; die sichere Variante wäre Verlängerung
  auf unbestimmte Zeit mit max. 1 Monat Kündigungsfrist. Entscheidung Max.


### 2026-08-13 · Developer Agent · Vola-Abruf an die Session gebunden (fail-closed)
- Ausgangsverdacht (Max): GVZ/OVX seit Wochen eingefroren, yfinance tot.
  **Diagnose widerlegt das:** Beide liefern täglich echte Werte, die an 11 von
  14 Tagen exakt dem Schlusskurs der jeweiligen Session entsprechen. Der
  CBOE-Fallback existierte bereits — und zwar für alle vier Vola-Indizes.
- Was tatsächlich kaputt war: `_yf_last()` nahm `Close.iloc[-1]`, **ohne das
  Datum des Bars zu prüfen**. Fehlte der Bar der letzten Session (yfinance
  veröffentlicht ihn mitunter verspätet oder gar nicht), kam der Schluss der
  Session davor still als tagesaktueller Wert durch — ein gültiger Wert, also
  griff der Fallback nie. Betroffen an 3 von 14 Tagen (27.07., 29.07., 04.08.)
  **alle vier** yfinance-Vola-Werte gleichzeitig, dazu VVIX am 12.08.
  (Lücke in der ^VVIX-Reihe). Das Dokument wies dabei einen Datenstand aus,
  den die Zahlen nicht hatten, mit Δ Vortag ±0,00.
- Was geändert: Jeder Vola-Wert muss jetzt **genau die letzte abgeschlossene
  US-Session** beschreiben (`_yf_session_close`, feiertagsfest über
  `dd_verify._expected_last_session`). Der CBOE-Fallback ordnet seinen Wert
  über `last_trade_time` einer Session zu, statt sie zu raten. Lässt sich die
  Frische nicht nachweisen → `value=None`, `verified=False`, Telegram-Alarm an
  Max; die Kundenausgabe zeigt einen Gedankenstrich statt einer alten Zahl.
  Zusätzlich Plausibilitätsbänder je Index (GVZ/OVX 5–150). Nebenbei
  behoben: Bei einem Lauf während des US-Handels wäre der laufende Tagesbar
  als Schlusskurs veröffentlicht worden.
- Betroffene Dateien: `bot/morning_routine.py`, `tests/test_vola_fetch.py` (neu)
- Status: fertig · Tests 404 grün (17 neue) · Live verifiziert: alle vier
  Indizes tragen Session 2026-08-12
- Für Sepp relevant: ja — Kennzahlen können in der Kundenausgabe jetzt
  bewusst leer bleiben (Gedankenstrich + Alarm) statt still zu veralten.
  Fehlt der VIX, blockt das bestehende Verifier-Gate die Auslieferung ganz.

### 2026-08-12 · UI Agent · Morgenroutine-PDF: Marke, Warnzeichen, Tacho, DD-Status
- Was geändert: Vier Design-Punkte am Kunden-PDF plus ein Konsistenz-Fix.
  (1) **Marke** — der Kopf trägt jetzt dieselbe Bilddatei wie der
  Website-Header (`web/static/favicon.svg`, dunkle Kachel mit Ampelpunkten)
  statt einer nur im PDF gezeichneten Kreis-Marke; Wortmarke in Inter/Navy.
  (2) **„ACHT" ersetzt** — der mittlere Ampelzustand ist ein amberfarbenes
  Warndreieck mit Ausrufezeichen; der Kurzcode entfällt ersatzlos, die Form
  trägt die Bedeutung (Graustufendruck bleibt lesbar).
  (3) **Sentiment als Tacho** — Fear & Greed und Put/Call erscheinen als
  Halbrund-Tacho mit denselben Zonen wie das Dashboard. Geometrie und Zonen
  liegen neu in `bot/gauge.py`; die Web-Komponente holt ihre `data-zones`
  von dort, das PDF zeichnet daraus statisches SVG (WeasyPrint kann kein JS).
  Der Fortschrittsbogen der Web-Variante fehlt bewusst — er überdeckt die
  Zonenbänder und widerspräche bei Put/Call der Skala.
  (4) **Distribution-Days-Textfeld** — Farbbalken und Status-Badge statt
  neutralem Kasten.
  **Konsistenz:** Der DD-Status kommt jetzt aus einer Funktion
  (`dd_policy.dd_status`) und gilt für Zählerpunkte, Badge, Zonenkante,
  Abschnittsmarke auf Seite 2, Fließtext und Veränderungstabelle. Vorher
  zeigte Seite 1 gelbe Zähler neben „Unauffällig" und Seite 2
  „ACHT · ERHÖHT" über „Keine auffällige Verteilung".
- Betroffene Dateien: `bot/gauge.py` (neu), `bot/dd_policy.py`,
  `bot/morning_payload.py`, `bot/morning_pdf.py`,
  `web/templates/morning_pdf_macros.html`, `web/templates/morning_pdf_b.html`,
  `web/templates/morning.html`, `web/static/css/morning_pdf.css`,
  `web/app.py`, `docs/morning_payload.schema.json` (+ Beispiel),
  `docs/morning_pdf_variante_b.pdf`, `tests/test_morning_pdf.py`
- Status: fertig und **live** · Tests 387 grün (9 neue) · Layout weiter
  deterministisch 2 Seiten, Reserve 7 mm im Referenzfall
- Nachtrag (Störung, behoben nach 1 h): Der Versuch, die Änderung bis zur
  Freigabe zurückzuhalten, indem die Dienste nicht neu gestartet werden,
  ging schief. **Jinja kompiliert jedes Template beim ersten Aufruf nach dem
  Start, nicht beim Start** (`auto_reload` ist aus, der Cache füllt sich
  lazy). `/morning` war seit dem letzten Neustart nicht aufgerufen worden —
  der erste Besuch lud damit die neue Datei, die die neuen Tacho-Globals
  aufruft, die im laufenden `app.py` noch fehlten → `UndefinedError`,
  `/morning` und das öffentliche `/beispiel` auf 500. Derselbe Riss hätte am
  Folgetag die 07:55-PDF-Erzeugung getroffen (alter Renderer, neue Makros).
  Beide Dienste neu gestartet, Website und Archiv-Download verifiziert.
  Konsequenz: Ein Template-Commit ist sofort produktiv — Zurückhalten geht
  nur über den Branch, nicht über den Neustart.
- Für Sepp relevant: ja — das Kundendokument ändert sein Erscheinungsbild ab
  der nächsten Ausgabe. Das bereits ausgelieferte Artefakt vom 12.08. ist
  unverändert (Rendering idempotent, Portal-Download byte-identisch geprüft).


### 2026-08-12 · Developer Agent · Abo-Selbstverwaltung: Kündigung zurücknehmbar
- Was geändert: Eine Kündigung war bisher eine Sackgasse — der Kunde konnte sie
  nicht zurücknehmen. Neu: `POST /abo/resume` nimmt die Kündigung zurück,
  solange die Laufzeit noch läuft (bei Stripe-Abos zuerst
  `cancel_at_period_end=False`, fail-closed bei Fehler), stellt das
  ursprüngliche Laufzeitende wieder her (neue Spalte
  `subscriptions.expires_at_pre_cancel` + Migration) und bestätigt per E-Mail.
  Abo-Seite jetzt zustandsklar mit genau einer Aktion je Zustand:
  aktiv → kündigen · gekündigt („Läuft aus") → Kündigung zurücknehmen ·
  ausgelaufen → Auslaufdatum + Neubuchung. Webhook-Reaktivierung räumt den
  Merker mit ab.
- Betroffene Dateien: `web/app.py`, `web/models.py`, `web/templates/abo.html`,
  `tests/test_kundenstrecke.py`
- Status: fertig · Tests 378 grün (5 neue), Dienste neu gestartet
- Für Sepp relevant: ja — Kunden können ihr Abo jetzt vollständig selbst
  steuern; Rückgewinnung ohne Support-Kontakt möglich.


### 2026-08-10 · Security Agent · Audit + Härtungsrunde
- Was geändert: Voll-Audit gegen die Security-Baseline (alle Checks bestanden),
  anschließend Härtungsrunde: CSP auf Nonce-basierte script-src umgestellt
  (Inline-Handler vollständig abgelöst, Regression testabgesichert),
  Dependency-Scan sauber + als wöchentlicher Routine-Job verankert, weitere
  Defense-in-Depth-Maßnahmen an App, Diensten und Reverse-Proxy. Baseline und
  Prüfskript fortgeschrieben (37 Checks). Details bewusst nicht hier
  (öffentlicher Mirror) — interne Doku im Security-Skill, Bericht an Max.
- Betroffene Dateien: `web/app.py`, `web/models.py`, `sepp_receiver.py`,
  `scheduler.py`, Templates, `tests/test_security_hardening.py` (neu),
  Security-Skill-Referenzen
- Status: fertig · Tests 373 grün (10 neue)
- Für Sepp relevant: ja — Security-Feinschliff aus der GoLive-Liste ist damit
  abgeschlossen; kein offener launch-blockierender Security-Punkt.


### 2026-08-10 · Developer Agent · Delivery Commit 2 abgeschlossen + Checkout-Legal
- Was geändert:
  - **Gestaffelter Tagesablauf** statt einem 08:00-Block: 07:45 Datenabruf ·
    07:50 Verifier-Gate · 07:55 Rendering · 08:00 Versand. Vorher fiel ein
    Gate-FAIL erst auf, wenn die Mail schon draußen war.
  - **Fail-closed-Versand:** kein Gate ⇒ kein PDF ⇒ **kein Versand** (Mail *und*
    Telegram), Alarm an Max, kein Fallback auf Vortagswerte. Bis dahin ging in
    dem Fall eine Mail ohne Anhang raus — also ungeprüfte Zahlen als Produkt.
  - **Versandprotokoll je Empfänger** (Status + Grund) für gezielten Nachversand.
    Nebenbefund behoben: vom SMTP-Server *abgelehnte* Empfänger galten bisher
    als zugestellt.
  - **Telegram liefert zusätzlich das PDF** (`sendDocument`).
  - **Dezimaltrennzeichen vereinheitlicht auf Komma** für alle Kundenkanäle.
    Anlass war keine Stilfrage: Die Kundenseite mischte deutsche Tausender
    (`1.234`) mit englischen Dezimalstellen (`18.70`).
  - **Checkout-Legal:** § 312j-Buttonbeschriftung „Zahlungspflichtig bestellen"
    (vorher „buchen" — genügt der Vorgabe nicht), Pflichtangaben unmittelbar
    über dem Button, zweite Pflicht-Checkbox AGB/Datenschutz serverseitig geprüft.
  - **Verifier-Abgrenzung dokumentiert** (`docs/verifier.md`): Das
    „4-Schichten-Gate" ist kein drittes Modul, sondern der Bauplan der beiden
    vorhandenen Gates. 3 von 4 Schichten stehen; die Lücke (Frozen-Detection
    für Nicht-DD-Kennzahlen) ist benannt statt stillschweigend offen.
  - **Kundenmail gekürzt** auf ein Anschreiben zum PDF; Betreff 45 statt 79
    Zeichen, ohne Emoji.
- Betroffene Dateien: `bot/morning_delivery.py` (neu), `bot/formatting.py` (neu),
  `docs/verifier.md` (neu), `scheduler.py`, `bot/morning_routine.py`,
  `bot/morning_mail.py`, `bot/morning_telegram.py`, `bot/morning_payload.py`,
  `bot/morning_pdf.py`, `bot/mailer.py`, `web/app.py`, `web/templates/abo.html`,
  `web/templates/base.html`, `web/templates/morning.html`,
  `tests/test_morning_delivery.py` (neu), `tests/test_kundenstrecke.py` (neu)
- Status: fertig · Tests 363 grün (35 neue)
- Für Sepp relevant: **ja** — Ziel 1 (Delivery) ist damit abgeschlossen, Ziel 2
  (Verifier) ist geklärt und abgegrenzt statt offen. Der verbleibende
  GoLive-Pfad liegt fast vollständig bei Max (Stripe-PriceID, DNS/SPF/DKIM/DMARC,
  Offsite-Backup, Uptime-Check, Beta-Nutzer) — siehe `TODO.md`.

### 2026-08-03 · Developer Agent · Morgenroutine-Delivery Commit 2 (Ziel 1)
- Was geändert: Auslieferung der Morgenroutine an Kunden vervollständigt —
  das Tages-PDF wird idempotent gerendert, an die Kunden-Mail **angehängt** und
  über ein geschütztes **Portal-Archiv** (`/morgenroutine`) zum Download
  bereitgestellt. Zugriff je Kunde über die `morning`-Subscription-Fenster
  (bildet Kündigungslücken korrekt ab) statt eines redundanten Datumsfelds.
- Sicherheit: Download über die App (kein Direktlink), striktes Datumsformat
  (Traversal-Schutz), Fensterprüfung (IDOR-Schutz) — 18 neue Tests grün.
- Betroffene Dateien: `bot/morning_pdf.py`, `bot/morning_mail.py`, `bot/mailer.py`,
  `bot/morning_routine.py`, `web/app.py`, `web/models.py`, `web/templates/*`
- Status: fertig · Für Sepp relevant: ja (Kern von Ziel 1 – Delivery steht)

### 2026-08-03 · Developer Agent · Morgenroutine ↔ Scanner getrennt (Level 1)
- Was geändert: Morgenroutine ist das Kundenprodukt; Scanner, Positionen und
  Journal sind strikt Max-only. Scanner-Dashboard (`/`) von paket- auf
  admin-gegated, „scanner" ist kein Kundenpaket mehr, Kunden-Home = Morgenroutine.
- Bestätigt: Die Morgenroutine benötigt **keinen** IB-Zugang (läuft bereits bei
  gestopptem Gateway) → für den Kunden-Launch bleibt das IB Gateway aus, was die
  Angriffsfläche deutlich verkleinert.
- Offen (Max' Entscheidung): Netz-Isolation der Admin-Oberfläche (Level 2).
- Status: fertig · Für Sepp relevant: ja (Produktabgrenzung Kunde ↔ Admin)

### 2026-08-03 · Security Agent · Härtung Stufe 2 umgesetzt (Ziel 3)
- Was geändert: Zusätzliche Härtungsschicht auf Web-App, Reverse-Proxy und
  Abhängigkeiten umgesetzt und verifiziert; ein aus dem Voll-Audit bekanntes
  akzeptiertes Restrisiko geschlossen. Dependency-Scan: 0 bekannte Schwachstellen.
- Details bewusst nicht hier (Öffentlichkeits-Regel) — Baseline im privaten Repo.
- Status: fertig · Für Sepp relevant: ja (Ziel 3 substanziell vorangebracht)

### 2026-08-03 · Developer Agent · Rechtstexte finalisiert (Ziel 4, kritischer Pfad)
- Was geändert: Datenschutz + Widerruf vollständig befüllt (alle Platzhalter aus
  vorhandenen Impressum-Stammdaten), Entwurf-Banner + „anwaltlich prüfen"-Marks
  entfernt. Impressum + AGB waren bereits vollständig. Alle 4 Rechtsseiten live
  (HTTP 200), Web-Service neu gestartet.
- Fakten eingesetzt: Aufsichtsbehörde BayLDA (Firmensitz Bayern);
  Server-Log-Aufbewahrung 14 Tage (verifiziert gegen logrotate daily/rotate 14);
  AVV Hetzner + IONOS als bestehende Standard-DPA formuliert.
- **Entscheidung Max: Finalisierung ohne Anwalt.** Restrisiko
  Geschäftsmodell-Einstufung (Informationsdienst vs. erlaubnispflichtige
  Anlagevermittlung/-beratung) liegt bewusst bei Max — Morning-only-Launch
  (rein beschreibend) ist der aufsichtsrechtlich defensivste Weg.
- Betroffene Dateien: `web/templates/datenschutz.html`, `web/templates/widerruf.html`
- Status: fertig · Für Sepp relevant: ja (Legal-Blocker Rechtstexte erledigt)

### 2026-08-03 · Developer Agent · Regelwerk konsolidiert (Sepp-Task 1: Fundament)
- Was geändert: Ein konsolidiertes, menschenlesbares Regelwerk als abgeleitete
  Referenz erstellt (`REGELWERK.md`); Quellen-Hierarchie fixiert: **CLAUDE.md +
  `config/settings.py` = Quelle**, alle weiteren Dokumente ziehen daraus nach.
- Divergenzen aufgelöst (Code gewinnt, Entscheidung Max 2026-08-03):
  - **DTE 45–70/55** verbindlich — Sepps „50–80/65" war ein Phantomwert
    (existierte nirgends; deckungsgleich mit Altentscheid 2026-07-08).
  - **CLAUDE.md Portfolio-Limit 6 → 4** (FOP 2 / Aktien 3) an `settings.py`
    angeglichen — Altwert, war nie im Code.
  - Kunden-Docx `docs/Scanner_Beschreibung.docx`: **Profit-Target 65 % → 60 %**
    korrigiert (Bug: Docx-Stand 21.07. lag 1 Tag vor der PT-Änderung), Datum
    aktualisiert. Trefferquote 65/35 unverändert (kein PT).
  - Altes Master-Regelwerk (24.06., Executor-/Windows-/12-Futures-/„nur-CL+GC"-
    Ballast) nach `docs/archiv/` verschoben + `README.md` mit Begründung.
- Konsistenz-Check: 19/19 Kernparameter `settings.py` ⟷ CLAUDE.md ⟷ REGELWERK.md grün.
- Betroffene Dateien: `REGELWERK.md` (neu), `CLAUDE.md`, `docs/Scanner_Beschreibung.docx`,
  `docs/archiv/…_ARCHIVIERT_24-06.docx`, `docs/archiv/README.md`, `STATUS.md`
- Status: fertig
- Für Sepp relevant: **ja** — `REGELWERK.md` ist ab jetzt der eine konsolidierte
  Regelwerk-Text; DTE-Finalstand ist 45–70/55, nicht 50–80/65.

### 2026-07-28 · Security Agent · Baseline-Audit (alle Checks grün)
- Auslöser: gemeldeter Verdacht auf zu weite sudo-Rechte des Dienst-Users.
  Ergebnis nach Prüfung: **Fehlalarm** – der Dienst-User ist unverändert eng
  gefasst (Least Privilege bestätigt), die Dienste laufen als non-root.
- Vollständiger Baseline-Check gefahren: alle Prüfungen bestanden. Eine kleine
  Header-Konsistenz-Härtung umgesetzt und das Audit-Tooling auf die aktuelle
  Produktions-Domain aktualisiert.
- Für Sepp relevant: nein (rein interne Härtung/Verifikation).

### 2026-07-28 · Developer Agent · Komplettes Web-Redesign (Finanzportal-Anmutung) — LIVE
- Auslöser (Max): der bisherige Look wirkte wie ein generisches KI-/Startup-Template
  (wie buyingmachine.io). Ziel: Anmutung eines sachlichen deutschen Finanzportals
  (Vorbild finanzen.net – als Anmutung, nicht kopiert).
- Landingpage (`landing.html`) komplett neu: dunkle Kopfleiste + Portal-Navigation,
  Kursticker-Zeile (als „Beispiel · Archiv" gekennzeichnet, keine Live-Kurse),
  linksbündiger zweispaltiger Hero, dichte Tabellen/Zeilen statt Kachel-Grid.
- Design-System (`base.html`) umgestellt — Token-Werte neu, Klassennamen gleich,
  daher ziehen alle App-Seiten automatisch mit: Portal-Blau #14568f, Navy #0c2540,
  Radius 4px, Schatten reduziert, Inter statt Space Grotesk, Verläufe/Glow entfernt.
- Eigenständige Seiten (register/login/customer_login/legal_base) auf dieselbe
  Palette gezogen; Inline-Alt-Farben in App-Templates nachgezogen.
- UI-Skill-Doku (`.claude/skills/steady-ground-ui/SKILL.md`) auf die neue
  Designsprache aktualisiert (Space Grotesk raus, flach/Portal verbindlich).
- PDF-Templates (`morning_pdf_*`) bewusst unberührt (eigenes Print-Design).
- Smoke-Test: alle 27 Templates fehlerfrei, öffentliche Routen 200. Nach Freigabe
  Max nach main gemergt und Webdienst neu gestartet → live.
- Für Sepp relevant: **ja** — öffentliches Erscheinungsbild komplett erneuert.

### 2026-07-27 · Developer Agent · Morgenroutine-PDF: Feinschliff + Variante B als finale Fassung
- **Entscheidung Max: nur noch Variante B** (kartenbasiert). Variante A verworfen —
  `morning_pdf_a.html` + Beispiel-PDF entfernt, Renderer/Tests/CSS auf B reduziert.
- Sechs Änderungswünsche Max umgesetzt:
  1. Zustandsfeld zeigt jetzt eine **echte dreilampige Ampel** (rot/gelb/grün,
     aktive Lampe leuchtet) statt eines Einzelsymbols.
  2. **Abschnitts-Nummerierung entfernt** (vorher 03–08 / 01–04, verwirrend).
  3. Spalte **„Einordnung" aus der Volatilitätstabelle entfernt**.
  4. **Alle Status-Symbole rund**, unabhängig von Rot/Gelb/Grün (Kurzcode
     OK/ACHT/RISK bleibt als nicht-farblicher Kanal).
  5. **Regelnummern im Regelwerk-Bezug weggelassen** (nur Titel + Text).
  6. **Verifier-Kasten in der Datenqualität entfernt**.
- Testversand am 27.07. an die Testuser (Max' eigene Postfächer) war erfolgt;
  heutige echte Daten (Sonntag) fielen korrekt durchs DD-Gate, daher als
  Vorschau die letzte gültige Ausgabe (24.07.) verschickt.
- Betroffene Dateien: `web/templates/morning_pdf_b.html`,
  `web/templates/morning_pdf_macros.html`, `web/static/css/morning_pdf.css`,
  `bot/morning_pdf.py`, `tests/test_morning_pdf.py`,
  `docs/morning_pdf_variante_b.pdf`, `docs/morning_payload.example.json`,
  `docs/morning_delivery.md` · entfernt: `web/templates/morning_pdf_a.html`,
  `docs/morning_pdf_variante_a.pdf`
- Tests: 28 grün (Variante-A-Fälle entfallen).
- Status: **fertig** · Für Sepp relevant: **ja** — Layout final, Commit 2
  (Scheduler/Versand/Kundenbereich) kann auf B aufsetzen.
...
- Was geändert: Token des neuen Kunden-Bots in secrets.env eingetragen (TELEGRAM_CUSTOMER_BOT_TOKEN). getMe ✓ („Steady Ground", @SteadyAlphaServiceBot), getUpdates-Polling ✓ — Telegram-Zustellung der Morgenroutine ist damit komplett einsatzbereit (Kanalwahl → /start beim Bot → automatische Verknüpfung → täglicher Versand). Logo-PNG an Max geschickt für BotFather-Profilbild.
...
- Was geändert: IONOS-SMTP in secrets.env konfiguriert (smtp.ionos.de, Port 587/STARTTLS statt 465 — Hetzner sperrt 465 ausgehend; Absender „Steady Ground <noreply@steadyalpha.de>"). Testmail an Max erfolgreich versendet. Damit automatisch scharf: Double-Opt-in-Bestätigungsmail bei Registrierung + tägliche Morgenmail an verifizierte Kunden mit E-Mail-Kanal. SPF/DKIM/DMARC via IONOS-Records der Domain vorhanden.
- Betroffene Dateien: keine im Repo (nur /etc/optionsbot/secrets.env)
- Status: fertig
...
- Was geändert (Auftrag Max, Basis: Skill `steady-ground-marketing`):
...
- Was geändert: Standard-Konvention `.claude/skills/` für Claude-Code-Skills angelegt; erste Skill `steady-ground-marketing` installiert (Positionierung/USP, Zielgruppe, Tonalität, regulatorische Leitplanken WpHG/KWG, Channel-Guidance, Agenten-Workflow). CLAUDE.md um Skills-Abschnitt ergänzt.
- Betroffene Dateien: `.claude/skills/steady-ground-marketing/SKILL.md`, `CLAUDE.md`
- Status: fertig
...
- Was geändert: Favicon (SVG), Branding „Steady Ground" auf allen Seiten, Responsive- und Security-Feinschliff (u. a. Security-Header, Meta-Tags).
...
- Status: fertig
- Für Sepp relevant: ja — erster öffentlicher Auftritt von Steady Ground.
